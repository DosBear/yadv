...
<xsl:stylesheet exclude-result-prefixes="ExDec" version="1.0" xmlns:ExDec="urn:customs.ru:Information:CustomsDocuments:GoodsExportDecision:5.10.0" xmlns:catSCVV="urn:customs.ru:SCVVCommonAggregateTypesCust:5.10.0" xmlns:cat_ru="urn:customs.ru:CommonAggregateTypes:5.10.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
   <!-- Решение о разрешении/запрете вывоза товаров -->
   ...
   <xsl:template match="ExDec:GoodsExportDecision">
      <html>
         <head>
            <title>
Решение о разрешении/запрете вывоза товаров
         ...
         <body>
            <!--На каждую партию - отдельная страница-->
            <xsl:for-each select="ExDec:ShipmentFactInfo">
               <div class="page">
                  <h2>Решение о разрешении/запрете вывоза партии №<xsl:value-of select="position()"/>
                  </h2>
                  <!--Основная таблица -->
                  <table>
                     <tbody>
				    ...
				    Дата и время принятия решения
                           ...
                           <td class="btext" style="width:100mm;">
                              <xsl:call-template name="russian_date">
                                 <xsl:with-param name="dateIn" select="//ExDec:GoodsExportDecision/ExDec:DecisionDate"/>
                              </xsl:call-template>
                              <xsl:text> </xsl:text>
                              <xsl:value-of select="substring(//ExDec:GoodsExportDecision/ExDec:DecisionDate, 12, 8)"/>
                           </td>
                        </tr>
                        <!--
				<xsl:for-each select="ExDec:ShipmentFactInfo">
					<tr>
						<td style="width:80mm;" class="b0">
						Партия №<xsl:value-of select="position()"/>
						</td>
						...
						Статус вывоза
                           ...
                           <td class="btext" style="width:100mm;">
                              <xsl:choose>
                                 <xsl:when test="catSCVV:StateShipment='GOODS_EXPORTED'">Товар вывезен</xsl:when>
                                 <xsl:when test="catSCVV:StateShipment='EXPORT_PERMIT_WITH_DISCR'">Вывоз разрешен. С расхождениями</xsl:when>
                                 <xsl:when test="catSCVV:StateShipment='EXPORT_BANNED'">Вывоз запрещен</xsl:when>
                                 <xsl:when test="catSCVV:StateShipment='EXPORT_PERMIT_WITHOUT CHECK'">Вывоз разрешен. Без контроля соответствия</xsl:when>
                                 <xsl:when test="catSCVV:StateShipment='EXPORT_CORRECTED'">Данные по вывозу откорректированы</xsl:when>
                              </xsl:choose>
                           </td>
                        </tr>
