...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:cat_ru="urn:customs.ru:CommonAggregateTypes:5.10.0" xmlns:clt_ru="urn:customs.ru:CommonLeafTypes:5.10.0" xmlns:mpsi="urn:customs.ru:Information:ExchangeDocuments:MPOStatInfo:5.13.3" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:mpco="urn:customs.ru:Information:CustomsDocuments:MPOCommon:5.13.3">
	<!-- Шаблон для типа MPOStatInfoType -->
	<xsl:template match="mpsi:MPOStatInfo">
		<html>
			<head>
				<style>
									body {
					...
					<table class="bordered w190">
						<tbody>
							<tr class="title">
								...
								<td class="graphMain bordered">Статистика по товарам, ранее поступивших получателю</td>
							</tr>
							<xsl:for-each select="mpsi:MPOList">
	...
	<xsl:template match="mpco:Consignee">
		<xsl:if test="cat_ru:OrganizationName">
			<table class="w190">
				<tr>
	...
	<!-- Шаблон для типа mpco:PrevGoodsType -->
	<xsl:template match="mpco:Goods">
		<tr>
			<td class="graphMain bordered">
				<xsl:value-of select="mpco:GoodsNumeric"/>
			</td>
			<td class="graphMain bordered">
				<xsl:value-of select="mpco:GoodsDescription"/>
			</td>
			<td class="graphMain bordered">
				<xsl:value-of select="translate(mpco:GrossWeight, '.', ',')"/>
			</td>
			<td class="graphMain bordered">
				<xsl:value-of select="translate(mpco:GoodsCostEuro, '.', ',')"/>
			</td>
			<td class="graphMain bordered">
				<xsl:call-template name="russian_date">
					<xsl:with-param name="dateIn" select="mpco:GoodsDate"/>
				</xsl:call-template>
			</td>
		</tr>
	</xsl:template>
	...
	<!-- Шаблон для типа mpsi:MPOListType -->
	<xsl:template match="mpsi:MPOList">
		<tr>
			<td class="graphMain bordered">
				<xsl:value-of select="mpsi:MPO_ID"/>
			</td>
			<td class="graphMain bordered">
				<xsl:value-of select="mpsi:RecNum"/>
			</td>
			<td class="graphMain bordered">
				<xsl:value-of select="mpsi:MPONumber"/>
			</td>
			<td class="graphMain bordered">
				<xsl:if test="mpsi:StatMonth">
					<xsl:apply-templates select="mpsi:StatMonth"/>
				</xsl:if>
			</td>
		</tr>
	</xsl:template>
	<!-- Шаблон для типа mpco:StatMonthType -->
	<xsl:template match="mpsi:StatMonth">
		<table class="w190">
			<tr>
				<td class="annot graphMain" style="width:50%">Признак наличия превышения по весу </td>
				<td class="value graphMain" style="width:50%">
					<xsl:choose>
						<xsl:when test="mpco:OverWeightSign='true'">
		...
		<table class="w190">
			<tr>
				<td class="annot graphMain" style="width:50%">Признак наличия превышения по стоимости</td>
				<td class="value graphMain" style="width:50%">
					<xsl:choose>
						<xsl:when test="mpco:OverCostSign='true'">
							<xsl:text>есть</xsl:text>
						</xsl:when>
						<xsl:otherwise>
							<xsl:text>нет</xsl:text>
		...
		<table class="w190">
			<tr>
				<td class="annot graphMain" style="width:50%">Стоимость товаров за календарный месяц, ранее поступивших в адрес получателя, евро</td>
				<td class="value graphMain" style="width:50%">
					<xsl:value-of select="translate(mpco:PreviousGoodsCost, '.', ',')"/>
				</td>
			</tr>
		</table>
		<table class="w190">
			<tr>
				<td class="annot graphMain" style="width:50%">Вес товаров, брутто, за календарный месяц, ранее поступивших в адрес получателя, кг</td>
				<td class="value graphMain" style="width:50%">
					<xsl:value-of select="translate(mpco:PreviousGoodsGrossWeight, '.', ',')"/>
				</td>
			</tr>
		</table>
		<table class="w190">
			<tr>
				<td class="annot graphMain" style="width:50%">Частота перемещения товаров в адрес получателя</td>
				<td class="value graphMain" style="width:50%">
					<xsl:value-of select="mpco:GoodsFrequency"/>
				</td>
			</tr>
		</table>
		<xsl:if test="mpco:Consignee">
			<div class="title marg-top">Получатель</div>
		</xsl:if>
		<xsl:apply-templates select="mpco:Consignee"/>
		<xsl:if test="mpco:Goods">
			<div class="title marg-top">Перечень ранее поставленных получателю товаров</div>
			<table class="bordered w190">
				<tbody>
					<tr class="title">
						...
						<td class="graphMain bordered">Дата поставки товара</td>
					</tr>
		...
		<xsl:if test="mpco:MPO_Registration">
			<div class="title marg-top">Регистрационные номера МПО</div>
			<table class="bordered w190">
				<tbody>
					<tr class="title">
						...
						<td class="graphMain bordered">Уникальный номер МПО</td>
					</tr>
					<xsl:for-each select="mpco:MPO_Registration">
						<xsl:apply-templates select="."/>
					</xsl:for-each>
				</tbody>
			</table>
