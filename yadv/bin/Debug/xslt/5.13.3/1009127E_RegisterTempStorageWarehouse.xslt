...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:clt_ru="urn:customs.ru:CommonLeafTypes:5.10.0" xmlns:cat_ru="urn:customs.ru:CommonAggregateTypes:5.10.0" xmlns:RUScat_ru="urn:customs.ru:RUSCommonAggregateTypes:5.13.3" xmlns:RUDECLcat="urn:customs.ru:RUDeclCommonAggregateTypesCust:5.13.3" xmlns:rtsw="urn:customs.ru:Information:SQDocuments:RegisterTempStorageWarehouse:5.13.3">
	<!-- Шаблон для типа RegisterTempStorageWarehouseType -->
	<xsl:template match="rtsw:RegisterTempStorageWarehouse">
		<html>
			<head>
				<style>
									body {
			...
			<body>
				<div class="page">
						...
						<tbody>
							<tr>
								<td align="center" class="graphMain">
									<font size="4">
										<b>Реестр владельцев складов временного хранения </b>
									</font>
								</td>
							</tr>
						</tbody>
					...
					<xsl:if test="rtsw:EECEDocHeaderAddInfo">
						<div class="title marg-t1">Дополнительные технологические реквизиты электронного документа (сведений)</div>
						<xsl:apply-templates select="rtsw:EECEDocHeaderAddInfo"/>
					</xsl:if>
					<div class="title marg-t1">Сведения об объекте реестра складов временного хранения</div>
					<table class="bordered w190">
						<xsl:for-each select="rtsw:RegisterTempStorageWarehouseDetails">
	...
	<xsl:template match="rtsw:EECEDocHeaderAddInfo">
	...
	<xsl:template match="rtsw:RegisterDocumentDetails">
		<table class="w190">
			<tr>
	...
	<xsl:template match="rtsw:RegisterOrganizationDetails">
	...
	<!-- Шаблон для типа rtsw:RegisterTempStorageWarehouseDetailsType -->
	<xsl:template match="rtsw:RegisterTempStorageWarehouseDetails">
		<tr>
			<td class="annot graphMain">
				<table class="w190">
					<tr>
						<td class="annot graphMain" style="width:40%">Кодовое обозначение страны, предоставившей информацию</td>
						<td class="value graphMain" style="width:60%"><xsl:value-of select="rtsw:RegisterCountryCode"/></td>
					</tr>
					<xsl:if test="rtsw:CustomsOfficeCode">
						<tr>
							<td class="annot graphMain" style="width:40%">Код таможенного органа</td>
							<td class="value graphMain" style="width:60%"><xsl:value-of select="rtsw:CustomsOfficeCode"/></td>
						</tr>
					</xsl:if>
					<xsl:apply-templates select="rtsw:ResourceItemStatusDetails"/>
				</table>
				<div class="title marg-top">Сведения о документе, подтверждающем включение юридического лица в реестр</div>
				<xsl:apply-templates select="rtsw:RegisterDocumentDetails"/>
				<div class="title marg-top">Сведения о реквизитах юридического лица, включенного в реестр</div>
				<xsl:apply-templates select="rtsw:RegisterOrganizationDetails"/>
				<div class="title marg-top">Сведения о складе временного хранения</div>
				<table class="bordered w190">
					<tr class="title">
					...
					<xsl:for-each select="rtsw:TSWDetails">
						<xsl:apply-templates select="."/>
					</xsl:for-each>
				</table>
				<div class="title marg-top">Период осуществления деятельности склада временного хранения</div>
				<table class="w190">
					<tr>
						<td class="annot graphMain" style="width:40%">1-й день действия, в том числе после временного прекращения / приостановления</td>
						<td class="value graphMain" style="width:60%">
							<xsl:call-template name="russian_date">
								<xsl:with-param name="dateIn" select="rtsw:StartActivityDate"/>
							</xsl:call-template>
						</td>
					</tr>
					<xsl:if test="rtsw:EndActivityDate">
						<tr>
							<td class="annot graphMain" style="width:40%">последний день действия, в том числе по причине временного прекращения</td>
							<td class="value graphMain" style="width:60%">
								<xsl:call-template name="russian_date">
									<xsl:with-param name="dateIn" select="rtsw:EndActivityDate"/>
								</xsl:call-template>
							</td>
						</tr>
					</xsl:if>
	...
	<xsl:template match="rtsw:ResourceItemStatusDetails">
	...
	<xsl:template match="rtsw:TSWDetails">
		<tr>
