<?xml version="1.0" encoding="utf-8"?>
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"  xmlns:voio="urn:customs.ru:Information:CustomsDocuments:VoiOrder:5.13.2" xmlns:xs="http://www.w3.org/2001/XMLSchema">
	<!-- Шаблон для типа VoiOrderType -->
	<xsl:template match="voio:VoiOrder">
		<html>
			<head>
				<meta content="text/html; charset=UTF-8" http-equiv="Content-Type"/>
				<style>
					@media print {
						div.page {
							border: none;
							margin:0;
							padding:0;
						}
					}

					body {
						background: #cccccc;
					}

					div.page {
						width: 210mm;
						margin: 10px auto;
						margin-top: 6pt;
						margin-bottom: 6pt;
						padding: 10mm 10mm 10mm 15mm;
						background: #ffffff;
						border: solid 1pt #000000;
						page-break-after: always;
						font-family: Verdana;
					}

					.marg-top {
						margin-top:5mm;
					}

					table {
						width: 100%;
						border: 0;
						empty-cells: show;
						border-collapse: collapse;
						margin-top: 1px;
						word-break: break-all; /*перенос длинных строк не выходя за ширину ячейки*/
					}

					table.border tr {
						border: 1px solid gray;
						padding: 5px;
					}

					table.border td {
						padding: 5px;
						border: solid 1px windowtext;
					}

					.graph {
						font-family: Verdana;
						font-size: 12pt;
					}

					.value {
						border-bottom: solid 1px DarkGrey;
					}

					div.title, tr.title td { font-weight:bold;  }

					.bordered { border-collapse: collapse; }

					td {vertical-align:bottom;}
					
					td.bordered {
						border: solid 1px windowtext;
					}

					.graphMainTitle {
						vertical-align:top;
						font-size: 12pt;
						font-family:Verdana;
						font-weight: bold;
					}

					.graphMainSubTitle {
						vertical-align:top;
						font-size: 11pt;
						font-family:Verdana;
						font-weight: bold;
					}

					.graphMainHd {
						vertical-align:top;
						font-size: 12pt;
						font-family:Verdana;
					}

					.graphMainSubHd {
						vertical-align:top;
						font-size: 10pt;
						font-family:Verdana;
					}

					.graphMainValue {
						vertical-align:bottom;
						font-style: italic;
						font-family:Verdana;
						font-size: 12pt;
					}

					td.bord_bottom {
						border-bottom: solid 1px DarkGrey;
					}
					
					.delim {
						height: 7pt;
					}

					.docheader
					{
					font-size: 20pt;
					font-family:Verdana;
					}			

					.docheader_number
					{
					font-size: 18pt;
					font-family:Verdana;
					}			

					.normal
					{
					font-size: 12pt;
					font-family:Verdana;
					}			

					.bold
					{
					font-weight: bold;
					font-family:Verdana;
					font-size: 12pt;
					}			

					.italic
					{
					font-style: italic;
					font-family:Verdana;
					font-size: 10pt;
					}			
					

				</style>
			</head>
			<body>
				<div class="page">
					<xsl:variable name="img_gto">
						<xsl:value-of select="''"/>
					</xsl:variable>				

					<table width="190">
						<tbody>
							<tr>
								<td align="left" valign="top" width="25%">
									<img>
										<xsl:attribute name="src">
											<xsl:value-of select="concat('data:image/jpeg;base64,',$img_gto)"/>
										</xsl:attribute>
									</img> 
								</td>
								<td align="center"  style="vertical-align:middle;" width="70%">
									<span class="docheader">Предписание на осмотр с ИДК</span>
								</td>
								<td align="left" valign="top" width="5%">
								</td>
							</tr>
						</tbody>
					</table>

					<table>
						<col width="47%" align="left" class="graphMainHd" valign="bottom"/>
						<col width="3%" align="left" class="graphMainHd" valign="bottom"/>
						<col width="1%" align="left" class="graphMainValue" valign="bottom"/>
						<col width="49%" align="left" class="graphMainValue" valign="bottom"/>
						<tbody>
							<tr>
								<td><br/></td>
								<td></td>
								<td></td>
								<td></td>
							</tr>
							<tr>
								<td colspan="4" class="graphMainTitle">Сведения о предписании на осмотр:</td>
							</tr>
							<tr class="delim"><td colspan="4"></td></tr>
							<xsl:apply-templates select="voio:InstrInfo"/>
						</tbody>
					</table>

					<table>
						<col width="47%" align="left" class="graphMainHd" valign="bottom"/>
						<col width="3%" align="left" class="graphMainHd" valign="bottom"/>
						<col width="1%" align="left" class="graphMainValue" valign="bottom"/>
						<col width="49%" align="left" class="graphMainValue" valign="bottom"/>
						<tbody>
							<tr>
								<td><br/></td>
								<td></td>
								<td></td>
								<td></td>
							</tr>
							<tr>
								<td colspan="4" class="graphMainTitle">Направление на осмотр:</td>
							</tr>
							<tr class="delim"><td colspan="4"></td></tr>
							<tr>
								<td>Регистрационный номер</td>
								<td></td>
								<td class="bord_bottom"></td>
								<td class="bord_bottom">
									<xsl:value-of select="voio:InstrNumber/*[local-name() = 'CustomsCode']"/>
									<xsl:text>/</xsl:text>
									<xsl:call-template name="gtd_date">
										<xsl:with-param name="dateIn" select="voio:InstrNumber/*[local-name() = 'RegistrationDate']"/>
									</xsl:call-template>
									<xsl:text>/</xsl:text>
									<xsl:value-of select="voio:InstrNumber/*[local-name() = 'GTDNumber']"/>
								</td>
							</tr>
						</tbody>
					</table>

					<xsl:if test="voio:ActNumber">
						<table>
							<col width="47%" align="left" class="graphMainHd" valign="bottom"/>
							<col width="3%" align="left" class="graphMainHd" valign="bottom"/>
							<col width="1%" align="left" class="graphMainValue" valign="bottom"/>
							<col width="49%" align="left" class="graphMainValue" valign="bottom"/>
							<tbody>
								<tr>
									<td><br/></td>
									<td></td>
									<td></td>
									<td></td>
								</tr>
								<tr>
									<td colspan="4" class="graphMainTitle">Акт осмотра с ИДК:</td>
								</tr>
								<tr class="delim"><td colspan="4"></td></tr>
								<tr>
									<td>Регистрационный номер</td>
									<td></td>
									<td class="bord_bottom"></td>
									<td class="bord_bottom">
										<xsl:value-of select="voio:ActNumber/*[local-name() = 'CustomsCode']"/>
										<xsl:text>/</xsl:text>
										<xsl:call-template name="gtd_date">
											<xsl:with-param name="dateIn" select="voio:ActNumber/*[local-name() = 'RegistrationDate']"/>
										</xsl:call-template>
										<xsl:text>/</xsl:text>
										<xsl:choose>
											<xsl:when test="string-length(voio:ActNumber/*[local-name() = 'GTDNumber']) = 7">
												<xsl:call-template name="ActNumber">
													<xsl:with-param name="GTDID" select="substring(voio:ActNumber/*[local-name() = 'GTDNumber'],2,6)"/>
												</xsl:call-template>
											</xsl:when>
											<xsl:otherwise>
												<xsl:call-template name="ActNumber">
													<xsl:with-param name="GTDID" select="voio:ActNumber/*[local-name() = 'GTDNumber']"/>
												</xsl:call-template>
											</xsl:otherwise>
										</xsl:choose>
									</td>
								</tr>
							</tbody>
						</table>
					</xsl:if>

					<xsl:if test="voio:PreviousInspection">
						<table>
							<col width="47%" align="left" class="graphMainHd" valign="bottom"/>
							<col width="3%" align="left" class="graphMainHd" valign="bottom"/>
							<col width="1%" align="left" class="graphMainValue" valign="bottom"/>
							<col width="49%" align="left" class="graphMainValue" valign="bottom"/>
							<tbody>
								<tr>
									<td><br/></td>
									<td></td>
									<td></td>
									<td></td>
								</tr>
								<tr>
									<td colspan="4" class="graphMainTitle">Акт ранее проведенного осмотра с ИДК:</td>
								</tr>
								<tr class="delim"><td colspan="4"></td></tr>
								<tr>
									<td>Регистрационный номер</td>
									<td></td>
									<td class="bord_bottom"></td>
									<td class="bord_bottom">
										<xsl:value-of select="voio:PreviousInspection/*[local-name() = 'CustomsCode']"/>
										<xsl:text>/</xsl:text>
										<xsl:call-template name="gtd_date">
											<xsl:with-param name="dateIn" select="voio:PreviousInspection/*[local-name() = 'RegistrationDate']"/>
										</xsl:call-template>
										<xsl:text>/</xsl:text>
										<xsl:choose>
											<xsl:when test="string-length(voio:PreviousInspection/*[local-name() = 'GTDNumber']) = 7">
												<xsl:call-template name="ActNumber">
													<xsl:with-param name="GTDID" select="substring(voio:PreviousInspection/*[local-name() = 'GTDNumber'],2,6)"/>
												</xsl:call-template>
											</xsl:when>
											<xsl:otherwise>
												<xsl:call-template name="ActNumber">
													<xsl:with-param name="GTDID" select="voio:PreviousInspection/*[local-name() = 'GTDNumber']"/>
												</xsl:call-template>
											</xsl:otherwise>
										</xsl:choose>
									</td>
								</tr>
							</tbody>
						</table>
					</xsl:if>

					<xsl:if test="voio:IDKCustoms">
						<table width="190">
							<col width="47%" align="left" class="graphMainHd" valign="bottom"/>
							<col width="3%" align="left" class="graphMainHd" valign="bottom"/>
							<col width="1%" align="left" class="graphMainValue" valign="bottom"/>
							<col width="49%" align="left" class="graphMainValue" valign="bottom"/>
							<tbody>
								<tr>
									<td><br/></td>
									<td></td>
									<td></td>
									<td></td>
								</tr>
								<tr>
									<td colspan="4" class="graphMainTitle">Осмотр назначен:</td>
								</tr>
								<xsl:apply-templates select="voio:IDKCustoms"/>
							</tbody>
						</table>
					</xsl:if>
					
					<xsl:if test="voio:AssInspectionDateTime">
						<table width="190">
							<col width="47%" align="left" class="graphMainHd" valign="bottom"/>
							<col width="3%" align="left" class="graphMainHd" valign="bottom"/>
							<col width="1%" align="left" class="graphMainValue" valign="bottom"/>
							<col width="49%" align="left" class="graphMainValue" valign="bottom"/>
							<tbody>
								<tr>
									<td></td>
									<td></td>
									<td></td>
									<td></td>
								</tr>
								<tr class="delim"><td colspan="4"></td></tr>
								<tr>
									<td>Назначенные дата и время осмотра</td>
									<td></td>
									<td class="bord_bottom"></td>
									<td class="bord_bottom">
										<xsl:call-template name="russian_datetime">
											<xsl:with-param name="dateIn" select="voio:AssInspectionDateTime"/>
										</xsl:call-template>
									</td>
								</tr>
							</tbody>
						</table>
					</xsl:if>

					<xsl:if test="voio:IDKType">
						<table width="190">
							<col width="47%" align="left" class="graphMainHd" valign="bottom"/>
							<col width="3%" align="left" class="graphMainHd" valign="bottom"/>
							<col width="1%" align="left" class="graphMainValue" valign="bottom"/>
							<col width="2%" align="left" class="graphMainValue" valign="bottom"/>
							<col width="20%" align="left" class="graphMainValue" valign="bottom"/>
							<col width="3%" align="left" class="graphMainValue" valign="bottom"/>
							<col width="24%" align="left" class="graphMainValue" valign="bottom"/>
							<tbody>
								<tr>
									<td></td>
									<td></td>
									<td></td>
									<td></td>
									<td></td>
									<td></td>
									<td></td>
								</tr>
								<tr class="delim"><td colspan="7"></td></tr>
								<tr>
									<td>Тип эксплуатируемого ИДК</td>
									<td></td>
									<td colspan="2">
										<xsl:choose>
											<xsl:when test="voio:IDKType='М' or voio:IDKType='м' or voio:IDKType='M' or voio:IDKType='m'">
												<div align="right" style="width:5mm;height:5mm;border:1px solid grey;">X</div>
											</xsl:when>
											<xsl:otherwise>
												<div align="right" style="width:5mm;height:5mm;border:1px solid grey;"> </div>
											</xsl:otherwise>
										</xsl:choose>
									</td>
									<td> - мобильный</td>
									<td>
										<xsl:choose>
											<xsl:when test="voio:IDKType='С' or voio:IDKType='с' or voio:IDKType='C' or voio:IDKType='c'">
												<div align="right" style="width:5mm;height:5mm;border:1px solid grey;">X</div>
											</xsl:when>
											<xsl:otherwise>
												<div align="right" style="width:5mm;height:5mm;border:1px solid grey;"> </div>
											</xsl:otherwise>
										</xsl:choose>
									</td>
									<td> - стационарный</td>
								</tr>
								<tr class="delim"><td colspan="7"></td></tr>
								<tr>
									<td>Уникальный идентификатор ИДК</td>
									<td></td>
									<td class="bord_bottom"></td>
									<td colspan="4" class="bord_bottom">
										<xsl:value-of select="voio:IDK_id"/>
									</td>
								</tr>
								<xsl:if test="voio:TypeScan">
									<tr class="delim"><td colspan="7"></td></tr>
									<tr>
										<td>Тип сканирования</td>
										<td></td>
										<td class="bord_bottom"></td>
										<td colspan="4" class="bord_bottom">
											<xsl:choose>
												<xsl:when test="voio:TypeScan = '1'">
													<xsl:value-of select="'сканирование объекта контроля'"/>
												</xsl:when>
												<xsl:when test="voio:TypeScan = '2'">
													<xsl:value-of select="'радиационный контроль'"/>
												</xsl:when>
												<xsl:when test="voio:TypeScan = '3'">
													<xsl:value-of select="'повторное сканирование'"/>
												</xsl:when>
												<xsl:when test="voio:TypeScan = '4'">
													<xsl:value-of select="'при проведении сканирования произошел сбой'"/>
												</xsl:when>
												<xsl:otherwise>
													<xsl:value-of select="voio:TypeScan"/>
												</xsl:otherwise>
											</xsl:choose>
										</td>
									</tr>
								</xsl:if>
							</tbody>
						</table>
					</xsl:if>

					<xsl:if test="voio:NumberCustomsZone or voio:CustomsZoneName">
						<table width="190">
							<col width="47%" align="left" class="graphMainHd" valign="bottom"/>
							<col width="3%" align="left" class="graphMainHd" valign="bottom"/>
							<col width="1%" align="left" class="graphMainValue" valign="bottom"/>
							<col width="49%" align="left" class="graphMainValue" valign="bottom"/>
							<tbody>
								<tr class="delim"><td colspan="4"></td></tr>
								<tr>
									<td></td>
									<td></td>
									<td></td>
									<td></td>
								</tr>
								<tr>
									<td>Зона таможенного контроля:</td>
								</tr>
								<xsl:if test="voio:NumberCustomsZone">
									<tr class="delim"><td colspan="4"></td></tr>
									<tr>
										<td>&#160;&#160;&#160;Номер зоны</td>
										<td></td>
										<td class="bord_bottom"></td>
										<td colspan="4" class="bord_bottom">
											<xsl:value-of select="voio:NumberCustomsZone"/>
										</td>
									</tr>
								</xsl:if>
								<xsl:if test="voio:CustomsZoneName">
									<tr class="delim"><td colspan="4"></td></tr>
									<tr>
										<td>&#160;&#160;&#160;Наименование зоны</td>
										<td></td>
										<td class="bord_bottom"></td>
										<td colspan="4" class="bord_bottom">
											<xsl:value-of select="voio:CustomsZoneName"/>
										</td>
									</tr>
								</xsl:if>
							</tbody>
						</table>
					</xsl:if>

					<xsl:if test="voio:VoiOfficer">
						<table width="190">
							<col width="47%" align="left" class="graphMainHd" valign="bottom"/>
							<col width="3%" align="left" class="graphMainHd" valign="bottom"/>
							<col width="1%" align="left" class="graphMainValue" valign="bottom"/>
							<col width="49%" align="left" class="graphMainValue" valign="bottom"/>
							<tbody>
								<tr class="delim"><td colspan="4"></td></tr>
								<tr>
									<td></td>
									<td></td>
									<td></td>
									<td></td>
								</tr>
								<tr>
									<td>Должностное лицо оператор ИДК:</td>
									<xsl:call-template name="CustomsOfficer">
										<xsl:with-param name="nodeCO" select="voio:VoiOfficer"/>
									</xsl:call-template>
								</tr>
							</tbody>
						</table>
					</xsl:if>

					<xsl:if test="voio:CustDocNumber">
						<table width="190">
							<col width="47%" align="left" class="graphMainHd" valign="bottom"/>
							<col width="3%" align="left" class="graphMainHd" valign="bottom"/>
							<col width="1%" align="left" class="graphMainValue" valign="bottom"/>
							<col width="49%" align="left" class="graphMainValue" valign="bottom"/>
							<tbody>
								<tr>
									<td><br/></td>
									<td></td>
									<td></td>
									<td></td>
								</tr>
								<tr>
									<td colspan="4" class="graphMainTitle">Номер таможенного документа, по которому<br/>необходимо произвести осмотр:</td>
								</tr>
								<tr class="delim"><td colspan="4"></td></tr>
								<xsl:for-each select="voio:CustDocNumber">
									<xsl:if test="voio:DocKind">
										<tr class="delim"><td colspan="4"></td></tr>
										<tr>
											<td>Тип документа</td>
											<td></td>
											<td class="bord_bottom"></td>
											<td class="bord_bottom">
												<xsl:choose>
													<xsl:when test="voio:DocKind='0'">уведомление о прибытии на таможенную территорию</xsl:when>
													<xsl:when test="voio:DocKind='1'">транзитная декларация</xsl:when>
													<xsl:when test="voio:DocKind='2'">книжка МДП</xsl:when>
													<xsl:when test="voio:DocKind='3'">номер ДО-1/описи</xsl:when>
													<xsl:when test="voio:DocKind='4'">ДТ</xsl:when>
													<xsl:when test="voio:DocKind='5'">уведомление об убытии</xsl:when>
													<xsl:when test="voio:DocKind='6'">иностранная транзитная декларация</xsl:when>
													<xsl:when test="voio:DocKind='7'">коносамент</xsl:when>
													<xsl:when test="voio:DocKind='8'">документ физического лица</xsl:when>
													<xsl:when test="voio:DocKind='9'">иное</xsl:when>
													<xsl:otherwise><xsl:value-of select="voio:DocKind"/></xsl:otherwise>
												</xsl:choose>
											</td>
										</tr>
									</xsl:if>
									<xsl:if test="voio:GTDID">
										<tr class="delim"><td colspan="4"></td></tr>
										<tr>
											<td>Регистрационный номер документа</td>
											<td></td>
											<td class="bord_bottom"></td>
											<td class="bord_bottom">
												<xsl:apply-templates select="voio:GTDID"/>
											</td>
										</tr>
									</xsl:if>
									<xsl:if test="voio:DocNumber">
										<tr class="delim"><td colspan="4"></td></tr>
										<tr>
											<td>Номер документа</td>
											<td></td>
											<td class="bord_bottom"></td>
											<td class="bord_bottom">
												<xsl:value-of select="voio:DocNumber/*[local-name() = 'DocNumber']"/>
												<xsl:if test="voio:DocNumber/*[local-name() = 'DocDate']">
												<text> от </text>
												<xsl:value-of select="concat(substring(voio:DocNumber/*[local-name() = 'DocDate'],9,2),'.',substring(voio:DocNumber/*[local-name() = 'DocDate'],6,2),'.',substring(voio:DocNumber/*[local-name() = 'DocDate'],1,4))"/>
												</xsl:if>
											</td>
										</tr>
									</xsl:if>
								</xsl:for-each>	
							</tbody>
						</table>
					</xsl:if>

					<xsl:if test="voio:InspTransportMeans">
						<xsl:apply-templates select="voio:InspTransportMeans"/>
					</xsl:if>
					<xsl:if test="voio:BillDoc">
						<table width="190">
							<tbody>
								<tr>
									<td><br/></td>
								</tr>
								<tr>
									<td class="graphMainTitle">Товаросопроводительные документы:</td>
								</tr>
								<tr class="delim"><td></td></tr>
							</tbody>
						</table>
						<table width="190" class="border">
							<col width="45%" align="left" valign="bottom"/>
							<col width="35%" align="left" valign="bottom"/>
							<col width="20%" align="left" valign="bottom"/>
							<tbody>
								<tr class="graphMainHd" align="center">
									<td style="vertical-align:middle;">Наименование документа</td>
									<td style="vertical-align:middle;">Номер документа</td>
									<td style="vertical-align:middle;">Дата<br/>документа</td>
								</tr>
								<xsl:for-each select="voio:BillDoc">
									<xsl:apply-templates select="."/>
								</xsl:for-each>
							</tbody>
						</table>
					</xsl:if>
				</div>
			</body>
		</html>
	</xsl:template>

	<!-- Шаблон для типа voio:CustomsType -->
	<xsl:template match="voio:Customs">
		<tr class="delim"><td colspan="4"></td></tr>
		<tr>
			<td>&#160;&#160;&#160;Код таможенного органа</td>
			<td></td>
			<td class="bord_bottom"></td>
			<td class="bord_bottom">
				<xsl:value-of select="voio:Code"/>
			</td>
		</tr>
		<xsl:if test="voio:OfficeName">
			<tr class="delim"><td colspan="4"></td></tr>
			<tr>
				<td>&#160;&#160;&#160;Наименование таможенного органа</td>
				<td></td>
				<td class="bord_bottom"></td>
				<td class="bord_bottom">
					<xsl:value-of select="voio:OfficeName"/>
				</td>
			</tr>
		</xsl:if>
	</xsl:template>

	<!-- Шаблон для типа voio:GTDIDType -->
	<xsl:template match="voio:GTDID| voio:InstrNumber|voio:GTDID">
		<xsl:value-of select="voio:CustomsCode"/>
		<xsl:text>/</xsl:text>
		<xsl:call-template name="gtd_date">
			<xsl:with-param name="dateIn" select="voio:RegistrationDate"/>
		</xsl:call-template>
		<xsl:text>/</xsl:text>
		<xsl:value-of select="voio:GTDNumber"/>
	</xsl:template>

	<!-- Шаблон для типа voio:ATOPhotoType -->
	<xsl:template match="voio:PhotoJPG|voio:PhotoRAW">
		<tr class="graphMainValue">
			<td class="italic">
				<xsl:value-of select="voio:PhotoID"/>
			</td>
			<td>
				<xsl:value-of select="voio:Comments"/>
			</td>
			<td>
				<img>
					<xsl:attribute name="src">
						<xsl:text>data:image/jpeg;base64,</xsl:text>
						<xsl:value-of select="voio:Preview" />
					</xsl:attribute>
				</img>
			</td>
		</tr>
	</xsl:template>
	<!-- Шаблон для типа voio:CustomsType -->
	<xsl:template match="voio:IDKCustoms">
		<tr class="delim"><td colspan="4"></td></tr>
		<tr>
			<td colspan="4">Таможенный орган:</td>
		</tr>
		<xsl:if test="voio:Code">
			<tr class="delim"><td colspan="4"></td></tr>
			<tr>
				<td>&#160;&#160;&#160;Код ТО</td>
				<td></td>
				<td class="bord_bottom"></td>
				<td class="bord_bottom">
					<xsl:value-of select="voio:Code"/>
				</td>
			</tr>
		</xsl:if>
		<xsl:if test="voio:OfficeName">
			<tr class="delim"><td colspan="4"></td></tr>
			<tr>
				<td>&#160;&#160;&#160;Наименование ТО</td>
				<td></td>
				<td class="bord_bottom"></td>
				<td class="bord_bottom">
					<xsl:value-of select="voio:OfficeName"/>
				</td>
			</tr>
		</xsl:if>
	</xsl:template>
	
	<!-- Шаблон для типа voio:DocumentBaseType -->
	<xsl:template match="voio:BillDoc">
		<tr class="graphMainValue">
			<td>
				<xsl:value-of select="voio:PrDocumentName"/>
			</td>
			<td>
				<xsl:value-of select="voio:PrDocumentNumber"/>
			</td>
			<td align="center">
				<xsl:call-template name="russian_datetime">
					<xsl:with-param name="dateIn" select="voio:PrDocumentDate"/>
				</xsl:call-template>
			</td>
		</tr>
	</xsl:template>
	
	<!-- Шаблон для типа catESAD_cu:ContainerNumberType -->
	<xsl:template match="voio:InspContainer">
		<tr class="graphMainValue">
			<td>
				<xsl:value-of select="./*[local-name() = 'ContainerIdentificaror']"/>
			</td>
			<td align="center">
				<xsl:value-of select="./*[local-name() = 'FullIndicator']"/>
			</td>
		</tr>
	</xsl:template>

	<!-- Шаблон для типа voio:InspTransportMeansType -->
	<xsl:template match="voio:InspTransportMeans">
		<table width="190">
			<col width="100%" align="left" class="graphMainHd" valign="bottom"/>
			<tbody>
				<tr>
					<td><br/></td>
				</tr>
				<tr>
					<td class="graphMainTitle">Транспортное средство/контейнеры:</td>
				</tr>
			</tbody>
		</table>
		<xsl:if test="voio:ObjType">
			<table width="190">
				<col width="100%" align="left" class="graphMainHd" valign="bottom"/>
				<tbody>
					<tr class="delim"><td></td></tr>
					<tr>
						<td class="graphMainSubTitle">&#160;&#160;&#160;Тип ТС:</td>
					</tr>
					<tr class="delim"><td></td></tr>
				</tbody>
			</table>
			<table class="w190">
				<tr class="graphMainValue">
					<td width="3%" align="center">
						<xsl:choose>
							<xsl:when test="voio:ObjType='0'">
								<div align="right" style="width:5mm;height:5mm;border:1px solid grey;">X</div>
							</xsl:when>
							<xsl:otherwise>
								<div align="right" style="width:5mm;height:5mm;border:1px solid grey;"> </div>
							</xsl:otherwise>
						</xsl:choose>
					</td>
					<td align="left" style="width:37%"> - грузовой (более 3,5 тонн)</td>
					<td width="3%" align="center">
						<xsl:choose>
							<xsl:when test="voio:ObjType='1'">
								<div align="right" style="width:5mm;height:5mm;border:1px solid grey;">X</div>
							</xsl:when>
							<xsl:otherwise>
								<div align="right" style="width:5mm;height:5mm;border:1px solid grey;"> </div>
							</xsl:otherwise>
						</xsl:choose>
					</td>
					<td align="left" style="width:37%"> - грузовой (менее 3,5 тонн)</td>
					<td width="3%" align="center">
						<xsl:choose>
							<xsl:when test="voio:ObjType='2'">
								<div align="right" style="width:5mm;height:5mm;border:1px solid grey;">X</div>
							</xsl:when>
							<xsl:otherwise>
								<div align="right" style="width:5mm;height:5mm;border:1px solid grey;"> </div>
							</xsl:otherwise>
						</xsl:choose>
					</td>
					<td align="left" style="width:17%"> - легковой</td>
				</tr>
				<tr class="delim" colspan="6"><td></td></tr>
				<tr class="graphMainValue">
					<td width="3%" align="center">
						<xsl:choose>
							<xsl:when test="voio:ObjType='3'">
								<div align="right" style="width:5mm;height:5mm;border:1px solid grey;">X</div>
							</xsl:when>
							<xsl:otherwise>
								<div align="right" style="width:5mm;height:5mm;border:1px solid grey;"> </div>
							</xsl:otherwise>
						</xsl:choose>
					</td>
					<td align="left" style="width:37%"> - пассажирский</td>
					<td width="3%" align="center">
						<xsl:choose>
							<xsl:when test="voio:ObjType='4'">
								<div align="right" style="width:5mm;height:5mm;border:1px solid grey;">X</div>
							</xsl:when>
							<xsl:otherwise>
								<div align="right" style="width:5mm;height:5mm;border:1px solid grey;"> </div>
							</xsl:otherwise>
						</xsl:choose>
					</td>
					<td align="left" style="width:37%"> - порожний</td>
					<td width="3%" align="center">
						<xsl:choose>
							<xsl:when test="voio:ObjType='9'">
								<div align="right" style="width:5mm;height:5mm;border:1px solid grey;">X</div>
							</xsl:when>
							<xsl:otherwise>
								<div align="right" style="width:5mm;height:5mm;border:1px solid grey;"> </div>
							</xsl:otherwise>
						</xsl:choose>
					</td>
					<td align="left" style="width:17%"> - иной</td>
				</tr>
			</table>
		</xsl:if>
		<xsl:if test="voio:TransportMeans">
			<xsl:apply-templates select="voio:TransportMeans"/>
		</xsl:if>
		<xsl:if test="../voio:Person">
			<xsl:apply-templates select="../voio:Person"/>
		</xsl:if>
		<xsl:if test="voio:InspContainer">
			<table width="190">
				<col width="100%" align="left" class="graphMainHd" valign="bottom"/>
				<tbody>
					<tr class="delim"><td></td></tr>
					<tr>
						<td class="graphMainSubTitle">&#160;&#160;&#160;Сведения о контейнерах:</td>
					</tr>
					<tr class="delim"><td></td></tr>
				</tbody>
			</table>
			<table width="190" class="border">
				<col width="55%" align="left"/>
				<col width="45%" align="left"/>
				<tbody>
					<tr class="graphMainHd" align="center">
						<td style="vertical-align:middle;">Номер (идентификатор) контейнера</td>
						<td>Признак заполнения контейнера.<br/><span class="graphMainSubHd">(1 - товар занимает весь контейнер,<br/>2- товар занимает часть контейнера)</span></td>
					</tr>
					<xsl:for-each select="voio:InspContainer">
						<xsl:apply-templates select="."/>
					</xsl:for-each>
				</tbody>
			</table>
		</xsl:if>
	</xsl:template>

	<!-- Шаблон для типа voio:TransportMeansType -->
	<xsl:template match="voio:TransportMeans">
		<table width="190">
			<col width="47%" align="left" class="graphMainHd" valign="bottom"/>
			<col width="3%" align="left" class="graphMainHd" valign="bottom"/>
			<col width="1%" align="left" class="graphMainValue" valign="bottom"/>
			<col width="49%" align="left" class="graphMainValue" valign="bottom"/>
			<tbody>
				<tr>
					<td><br/></td>
					<td></td>
					<td></td>
					<td></td>
				</tr>
				<tr>
					<td colspan="4" class="graphMainSubTitle">&#160;&#160;&#160;Сведения о транспортном средстве:</td>
				</tr>
				<xsl:if test="voio:TransportModeCode">
					<tr class="delim"><td colspan="4"></td></tr>
					<tr>
						<td>Код вида ТС</td>
						<td></td>
						<td class="bord_bottom"></td>
						<td class="bord_bottom">
							<xsl:value-of select="voio:TransportModeCode"/>
						</td>
					</tr>
				</xsl:if>
				<xsl:if test="voio:VIN">
					<tr class="delim"><td colspan="4"></td></tr>
					<tr>
						<td>Номер шасси (VIN)</td>
						<td></td>
						<td class="bord_bottom"></td>
						<td class="bord_bottom">
							<xsl:value-of select="voio:VIN"/>
						</td>
					</tr>
				</xsl:if>
				<xsl:if test="voio:TransportKindCode">
					<tr class="delim"><td colspan="4"></td></tr>
					<tr>
						<td>Код типа транспортного средства</td>
						<td></td>
						<td class="bord_bottom"></td>
						<td class="bord_bottom">
							<xsl:value-of select="voio:TransportKindCode"/>
						</td>
					</tr>
				</xsl:if>
				<xsl:if test="voio:TransportMarkCode">
					<tr class="delim"><td colspan="4"></td></tr>
					<tr>
						<td>Код марки транспортного средства</td>
						<td></td>
						<td class="bord_bottom"></td>
						<td class="bord_bottom">
							<xsl:value-of select="voio:TransportMarkCode"/>
						</td>
					</tr>
				</xsl:if>
				<xsl:if test="voio:TransportIdentifier">
					<tr class="delim"><td colspan="4"></td></tr>
					<tr>
						<td>Номер ТС</td>
						<td></td>
						<td class="bord_bottom"></td>
						<td class="bord_bottom">
							<xsl:value-of select="voio:TransportIdentifier"/>
						</td>
					</tr>
				</xsl:if>
				<xsl:if test="voio:TransportMeansNationalityCode">
					<tr class="delim"><td colspan="4"></td></tr>
					<tr>
						<td>Код страны принадлежности ТС</td>
						<td></td>
						<td class="bord_bottom"></td>
						<td class="bord_bottom">
							<xsl:value-of select="voio:TransportMeansNationalityCode"/>
						</td>
					</tr>
				</xsl:if>
				<xsl:if test="voio:ActiveTransportIdentifier">
					<tr class="delim"><td colspan="4"></td></tr>
					<tr>
						<td>Идентификатор активного ТС<br/>для прицепов, полуприцепов и др.</td>
						<td></td>
						<td class="bord_bottom"></td>
						<td class="bord_bottom">
							<xsl:value-of select="voio:ActiveTransportIdentifier"/>
						</td>
					</tr>
				</xsl:if>
				<xsl:if test="voio:TransportRegNumber">
					<tr class="delim"><td colspan="4"></td></tr>
					<tr>
						<td>Номер свидетельства о регистрации<br/>ТС для РБ</td>
						<td></td>
						<td class="bord_bottom"></td>
						<td class="bord_bottom">
							<xsl:value-of select="voio:TransportRegNumber"/>
						</td>
					</tr>
				</xsl:if>
				<xsl:if test="voio:TrailerIdentifier">
					<tr class="delim"><td colspan="4"></td></tr>
					<tr>
						<td>Регистрационный номер<br/>прицепа/полуприцепа</td>
						<td></td>
						<td class="bord_bottom"></td>
						<td class="bord_bottom">
							<xsl:for-each select="voio:TrailerIdentifier"><xsl:value-of select="."/></xsl:for-each>
						</td>
					</tr>
				</xsl:if>
			</tbody>
		</table>
	</xsl:template>
	
	<!-- Шаблон для типа voio:CUCustomsType -->
	<xsl:template match="voio:Customs">
		<tr class="delim"><td colspan="4"></td></tr>
		<tr>
			<td>&#160;&#160;&#160;Код таможенного органа</td>
			<td></td>
			<td class="bord_bottom"></td>
			<td class="bord_bottom">
				<xsl:value-of select="voio:Code"/>
			</td>
		</tr>
		<xsl:if test="voio:OfficeName">
			<tr class="delim"><td colspan="4"></td></tr>
			<tr>
				<td>&#160;&#160;&#160;Наименование таможенного органа</td>
				<td></td>
				<td class="bord_bottom"></td>
				<td class="bord_bottom">
					<xsl:value-of select="voio:OfficeName"/>
				</td>
			</tr>
		</xsl:if>
		<xsl:if test="voio:CustomsCountryCode">
			<tr class="delim"><td colspan="4"></td></tr>
			<tr>
				<td>&#160;&#160;&#160;Код государства - члена ЕАЭС</td>
				<td></td>
				<td class="bord_bottom"></td>
				<td class="bord_bottom">
					<xsl:value-of select="voio:CustomsCountryCode"/>
				</td>
			</tr>
		</xsl:if>
	</xsl:template>

	<!-- Шаблон для типа voio:GTDIDType -->
	<xsl:template name="GTDID">
		<xsl:param name="DocName"/>
		<xsl:param name="DocNode"/>
		<xsl:value-of select="$DocNode/*[local-name() = 'CustomsCode']"/>
		<xsl:text>/</xsl:text>
		<xsl:call-template name="gtd_date">
			<xsl:with-param name="dateIn" select="$DocNode/*[local-name() = 'RegistrationDate']"/>
		</xsl:call-template>
		<xsl:text>/</xsl:text>
		<xsl:value-of select="$DocNode/*[local-name() = 'GTDNumber']"/>
	</xsl:template>
	
	<!-- Шаблон для типа voio:InstrInfoType -->
	<xsl:template match="voio:InstrInfo">

		<tr class="delim"><td colspan="4"></td></tr>
		<tr>
			<td>Дата и время создания документа</td>
			<td></td>
			<td class="bord_bottom"></td>
			<td class="bord_bottom">
				<xsl:call-template name="russian_date">
					<xsl:with-param name="dateIn" select="voio:InstrDate"/>
				</xsl:call-template>
				<xsl:text>   </xsl:text>
				<xsl:value-of select="substring(voio:InstrTime,1,8)"/>
			</td>
		</tr>

		<xsl:if test="voio:Customs">
			<tr class="delim"><td colspan="4"></td></tr>
			<tr>
				<td colspan="4">Таможенный орган:</td>
			</tr>
			<xsl:apply-templates select="voio:Customs"/>
		</xsl:if>

		<xsl:if test="voio:CustomsOfficer">
			<tr class="delim"><td colspan="4"></td></tr>
			<tr>
				<td>Должностное лицо ТО:</td>
				<xsl:call-template name="CustomsOfficer">
					<xsl:with-param name="nodeCO" select="./voio:CustomsOfficer"/>
				</xsl:call-template>
			</tr>
		</xsl:if>

	</xsl:template>
	
	<!-- Шаблон для типа voio:CustomsPersonDetails -->
	<xsl:template name="CustomsOfficer">
		<xsl:param name="nodeCO"/>
		
		<tr class="delim"><td colspan="4"></td></tr>
		<tr>
			<td>&#160;&#160;&#160;ФИО</td>
			<td></td>
			<td class="bord_bottom"></td>
			<td colspan="4" class="bord_bottom">
				<xsl:value-of select="$nodeCO/voio:PersonSurname"/>
				<xsl:text> </xsl:text>
				<xsl:value-of select="$nodeCO/voio:PersonName"/>
				<xsl:if test="$nodeCO/voio:PersonMiddleName">										
					<xsl:text> </xsl:text>
					<xsl:value-of select="$nodeCO/voio:PersonMiddleName"/>
				</xsl:if>
			</td>
		</tr>
		<xsl:if test="$nodeCO/voio:PersonPost">
			<tr class="delim"><td colspan="4"></td></tr>
			<tr>
				<td>&#160;&#160;&#160;Должность</td>
				<td></td>
				<td class="bord_bottom"></td>
				<td colspan="4" class="bord_bottom">
					<xsl:value-of select="$nodeCO/voio:PersonPost"/>
				</td>
			</tr>
		</xsl:if>
		<xsl:if test="$nodeCO/voio:LNP or $nodeCO/voio:CustomsCode">
			<tr class="delim"><td colspan="$colcnt+3"></td></tr>
			<tr>
				<xsl:choose>
					<xsl:when test="$nodeCO/voio:LNP and $nodeCO/voio:CustomsCode">
						<td>&#160;&#160;&#160;ЛНП (код ТО на оттиске ЛНП)</td>
					</xsl:when>
					<xsl:when test="$nodeCO/voio:LNP">
						<td>&#160;&#160;&#160;ЛНП</td>
					</xsl:when>
					<xsl:when test="$nodeCO/voio:CustomsCode">
						<td>&#160;&#160;&#160;Код ТО на оттиске ЛНП</td>
					</xsl:when>
				</xsl:choose>
				<td></td>
				<td class="bord_bottom"></td>
				<td colspan="4" class="bord_bottom">
					<xsl:if test="$nodeCO/voio:LNP">
						<xsl:value-of select="$nodeCO/voio:LNP"/>
						<xsl:if test="$nodeCO/voio:CustomsCode">
							<xsl:text> (</xsl:text>
						</xsl:if>
					</xsl:if>
					<xsl:if test="$nodeCO/voio:CustomsCode">
						<xsl:value-of select="$nodeCO/voio:CustomsCode"/>
						<xsl:if test="$nodeCO/voio:LNP">
							<xsl:text>)</xsl:text>
						</xsl:if>
					</xsl:if>
				</td>
			</tr>
		</xsl:if>
	</xsl:template>

	<!-- Шаблон для типа voio:PersonType -->
	<xsl:template match="voio:Person">
		<table width="190">
			<col width="47%" align="left" class="graphMainHd" valign="bottom"/>
			<col width="3%" align="left" class="graphMainHd" valign="bottom"/>
			<col width="1%" align="left" class="graphMainValue" valign="bottom"/>
			<col width="49%" align="left" class="graphMainValue" valign="bottom"/>
			<tbody>
				<tr>
					<td></td>
					<td></td>
					<td></td>
					<td></td>
				</tr>
				<tr class="delim"><td colspan="4"></td></tr>
				<tr>
					<td>Физическое лицо, управляющее ТС:</td>
				</tr>
				<tr>
					<td>&#160;&#160;&#160;ФИО</td>
					<td></td>
					<td class="bord_bottom"></td>
					<td colspan="4" class="bord_bottom">
						<xsl:value-of select="voio:PersonSurname"/>
						<xsl:text> </xsl:text>
						<xsl:value-of select="voio:PersonName"/>
						<xsl:if test="voio:PersonMiddleName">										
							<xsl:text> </xsl:text>
							<xsl:value-of select="voio:PersonMiddleName"/>
						</xsl:if>
					</td>
				</tr>
				<xsl:if test="voio:IdentityCardCode">
					<tr class="delim"><td colspan="4"></td></tr>
					<tr>
						<td>&#160;&#160;&#160;Код вида документа,<br/>&#160;&#160;&#160;удостоверяющего личность</td>
						<td></td>
						<td class="bord_bottom"></td>
						<td class="bord_bottom">
							<xsl:value-of select="voio:IdentityCardCode"/>
						</td>
					</tr>
				</xsl:if>
				<xsl:if test="voio:IdentityCardName">
					<tr class="delim"><td colspan="4"></td></tr>
					<tr>
						<td>&#160;&#160;&#160;Краткое наименование документа,<br/>&#160;&#160;&#160;удостоверяющего личность</td>
						<td></td>
						<td class="bord_bottom"></td>
						<td class="bord_bottom">
							<xsl:value-of select="voio:IdentityCardName"/>
						</td>
					</tr>
				</xsl:if>
				<xsl:if test="voio:IdentityCardSeries">
					<tr class="delim"><td colspan="4"></td></tr>
					<tr>
						<td>&#160;&#160;&#160;Серия документа</td>
						<td></td>
						<td class="bord_bottom"></td>
						<td class="bord_bottom">
							<xsl:value-of select="voio:IdentityCardSeries"/>
						</td>
					</tr>
				</xsl:if>
				<xsl:if test="voio:IdentityCardNumber">
					<tr class="delim"><td colspan="4"></td></tr>
					<tr>
						<td>&#160;&#160;&#160;Номер документа</td>
						<td></td>
						<td class="bord_bottom"></td>
						<td class="bord_bottom">
							<xsl:value-of select="voio:IdentityCardNumber"/>
						</td>
					</tr>
				</xsl:if>
				<xsl:if test="voio:IdentityCardDate">
					<tr class="delim"><td colspan="4"></td></tr>
					<tr>
						<td>&#160;&#160;&#160;Дата выдачи документа</td>
						<td></td>
						<td class="bord_bottom"></td>
						<td class="bord_bottom">
							<xsl:call-template name="russian_date">
								<xsl:with-param name="dateIn" select="voio:IdentityCardDate"/>
							</xsl:call-template>
						</td>
					</tr>
				</xsl:if>
			</tbody>
		</table>		
	</xsl:template>

	<xsl:template name="russian_datetime">
		<xsl:param name="dateIn"/>
		<xsl:choose>
			<xsl:when test="substring($dateIn,5,1)='-' and substring($dateIn,8,1)='-'">
				<xsl:value-of select="substring($dateIn,9,2)"/>
				<xsl:text>.</xsl:text>
				<xsl:value-of select="substring($dateIn,6,2)"/>
				<xsl:text>.</xsl:text>
				<xsl:value-of select="substring($dateIn,1,4)"/>
				<xsl:text>   </xsl:text>
				<xsl:value-of select="substring($dateIn,12,8)"/>
			</xsl:when>
			<xsl:otherwise>
				<xsl:value-of select="$dateIn"/>
			</xsl:otherwise>
		</xsl:choose>
	</xsl:template>

	<xsl:template name="gtd_date">
		<xsl:param name="dateIn"/>
		<xsl:choose>
			<xsl:when test="substring($dateIn,5,1)='-' and substring($dateIn,8,1)='-'">
				<xsl:value-of select="substring($dateIn,9,2)"/>
				<xsl:value-of select="substring($dateIn,6,2)"/>
				<xsl:value-of select="substring($dateIn,3,2)"/>
			</xsl:when>
			<xsl:otherwise>
				<xsl:value-of select="$dateIn"/>
			</xsl:otherwise>
		</xsl:choose>
	</xsl:template>

	<xsl:template name="russian_date">
		<xsl:param name="dateIn"/>
		<xsl:choose>
			<xsl:when test="substring($dateIn,5,1)='-' and substring($dateIn,8,1)='-'">
				<xsl:value-of select="substring($dateIn,9,2)"/>
				<xsl:text>.</xsl:text>
				<xsl:value-of select="substring($dateIn,6,2)"/>
				<xsl:text>.</xsl:text>
				<xsl:value-of select="substring($dateIn,1,4)"/>
			</xsl:when>
			<xsl:otherwise>
				<xsl:value-of select="$dateIn"/>
			</xsl:otherwise>
		</xsl:choose>
	</xsl:template>
	
	<xsl:template name="ActNumber">
		<xsl:param name="GTDID"/>
		<xsl:choose>
			<xsl:when test="string-length($GTDID) &lt; 6">
				<xsl:variable name="temp" select="concat('0',$GTDID)"/>
				<xsl:call-template name="ActNumber">
					<xsl:with-param name="GTDID" select="$temp"/>
				</xsl:call-template>
			</xsl:when>
			<xsl:otherwise>
				<xsl:value-of select="$GTDID"/>
			</xsl:otherwise>
		</xsl:choose>
	</xsl:template>
	
	
</xsl:stylesheet>
