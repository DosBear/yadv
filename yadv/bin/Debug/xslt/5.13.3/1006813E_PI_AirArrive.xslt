                  ...
                  <tbody>
                     ...
                     <!--xsl:if test="piair:CustomsCode"-->
						 <tr>
							<td colspan="2">Код таможенного органа
								<br/>
								<span class="text"><xsl:value-of select="piair:CustomsCode"/></span>
							</td>
							<td colspan="3">УИНП
								<br/>
								<span class="text"><xsl:value-of select="piair:UINP"/></span>
							</td>
						</tr>
                     <!--/xsl:if-->
                     <tr>
                     ...
                     <xsl:for-each select="piair:CargoManifest">
                        ...
                        <xsl:for-each select="piair:GoodsInfo">
                           ...
                           <tr>
                              <td>
								  <xsl:text>Торговое название товара: </xsl:text><xsl:value-of select="piair:GoodsName"/>
                     ...
                     <xsl:for-each select="piair:GTDID">
						<tr>
							<td colspan="5">
								...
								<xsl:text>/</xsl:text>
								<xsl:call-template name="number_date"><xsl:with-param name="dateIn" select="cat_ru:RegistrationDate"/></xsl:call-template>
								<xsl:text>/</xsl:text>
