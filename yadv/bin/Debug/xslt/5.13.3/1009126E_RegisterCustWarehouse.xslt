...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:clt_ru="urn:customs.ru:CommonLeafTypes:5.10.0" xmlns:cat_ru="urn:customs.ru:CommonAggregateTypes:5.10.0" xmlns:RUScat_ru="urn:customs.ru:RUSCommonAggregateTypes:5.13.3" xmlns:RUDECLcat="urn:customs.ru:RUDeclCommonAggregateTypesCust:5.13.3" xmlns:rcwar="urn:customs.ru:Information:SQDocuments:RegisterCustWarehouse:5.13.3">
	<!-- Шаблон для типа RegisterCustWarehouseType -->
	<xsl:template match="rcwar:RegisterCustWarehouse">
		<html>
			<head>
				<style>
									body {
			...
			<body>
				<div class="page">
						...
						<tbody>
							<tr>
								<td align="center" class="graphMain">
									<font size="4">
										<b>Общий реестр владельцев таможенных складов </b>
									</font>
								</td>
							</tr>
						</tbody>
					...
					<xsl:if test="rcwar:EECEDocHeaderAddInfo">
						<div class="title marg-t1">Дополнительные технологические реквизиты электронного документа (сведений)</div>
						<xsl:apply-templates select="rcwar:EECEDocHeaderAddInfo"/>
					</xsl:if>
					<div class="title marg-t1">Сведения об объекте реестра владельцев таможенных складов</div>
					<table class="bordered w190">
						<xsl:for-each select="rcwar:RegisterCustWarehouseDetails">
	...
	<xsl:template match="RUScat_ru:CubageMeasure| RUScat_ru:TotalAreaMeasure">
	...
	<!-- Шаблон для типа RUScat_ru:WarehouseDetailsType -->
	<xsl:template match="rcwar:CustomsWarehouseDetails">
		<tr>
			<td class="graphMain bordered"><xsl:apply-templates select="RUScat_ru:AddressDetails"/></td>
			<td class="graphMain bordered">
				<xsl:choose>
					<xsl:when test="RUScat_ru:WarehouseKindCode=1">склад временного хранения</xsl:when>
					<xsl:when test="RUScat_ru:WarehouseKindCode=2">таможенный склад</xsl:when>
					<xsl:when test="RUScat_ru:WarehouseKindCode=3">свободный склад</xsl:when>
				</xsl:choose>
			</td>
			<td class="graphMain bordered">
				<xsl:choose>
					<xsl:when test="RUScat_ru:TypeWarehouseIndicator='true' or RUScat_ru:TypeWarehouseIndicator='1'">закрытый</xsl:when>
					<xsl:when test="RUScat_ru:TypeWarehouseIndicator='false' or RUScat_ru:TypeWarehouseIndicator='0'">открытый</xsl:when>
				</xsl:choose>
			</td>
			<td class="graphMain bordered"><xsl:apply-templates select="RUScat_ru:TotalAreaMeasure"/></td>
			<td class="graphMain bordered"><xsl:apply-templates select="RUScat_ru:CubageMeasure"/></td>
			<td class="graphMain bordered">
				<xsl:for-each select="RUScat_ru:TransportModeCode">
					<xsl:value-of select="."/>
					<xsl:if test="position()!=last()">, </xsl:if>
				</xsl:for-each>
			</td>
			<td class="graphMain bordered">
				<xsl:for-each select="RUScat_ru:WarehouseActivityDescriptionText"><xsl:value-of select="."/></xsl:for-each>
			</td>
			<td class="graphMain bordered">
				<xsl:for-each select="RUScat_ru:AdditionalInfoText"><xsl:value-of select="."/></xsl:for-each>
			</td>
		</tr>
	</xsl:template>
	<!-- Шаблон для типа RUScat_ru:EECEDocHeaderAddInfoType -->
	<xsl:template match="rcwar:EECEDocHeaderAddInfo">
	...
	<!-- Шаблон для типа rcwar:RegisterCustWarehouseDetailsType -->
	<xsl:template match="rcwar:RegisterCustWarehouseDetails">
		<tr>
			<td class="annot graphMain">
				<table class="w190">
					<tr>
						<td class="annot graphMain" style="width:40%">Кодовое обозначение страны, предоставившей информацию</td>
						<td class="value graphMain" style="width:60%"><xsl:value-of select="rcwar:RegisterCountryCode"/></td>
					</tr>
					<xsl:if test="rcwar:CustomsOfficeCode">
						<tr>
							<td class="annot graphMain" style="width:40%">Код таможенного органа</td>
							<td class="value graphMain" style="width:60%"><xsl:value-of select="rcwar:CustomsOfficeCode"/></td>
						</tr>
					</xsl:if>
					<xsl:apply-templates select="rcwar:ResourceItemStatusDetails"/>
				</table>
				<div class="title marg-top">Сведения о документе, подтверждающем включение юридического лица в реестр</div>
				<xsl:apply-templates select="rcwar:RegisterDocumentDetails"/>
				<div class="title marg-top">Сведения о реквизитах юридического лица, включенного в реестр</div>
				<xsl:apply-templates select="rcwar:RegisterOrganizationDetails"/>
				<div class="title marg-top">Сведения о таможенном складе</div>
				<table class="bordered w190">
					<tr class="title">
						<td class="graphMain bordered">Адрес регистрации</td>
						<td class="graphMain bordered">Назначение склада</td>
						<td class="graphMain bordered">Тип склада</td>
						<td class="graphMain bordered">Общая площадь (кв. м)</td>
						<td class="graphMain bordered">Объем помещения (куб. м)</td>
						<td class="graphMain bordered">Код вида ТС</td>
						<td class="graphMain bordered">Сведения о виде деятельности, осуществляемой на территории склада</td>
						<td class="graphMain bordered">Дополнительная информация</td>
					</tr>
					<xsl:for-each select="rcwar:CustomsWarehouseDetails">
						<xsl:apply-templates select="."/>
					</xsl:for-each>
				</table>
				<xsl:if test="rcwar:StayActivityDateDetails">
					<div class="title marg-top">Сведения о временном прекращении/приостановлении осуществления деятельности юр. лица в качестве владельца таможенного склада</div>
					<table class="w190">
						<tr>
							<td class="annot graphMain" style="width:40%">Деятельность прекращена/приостановлена</td>
							<td class="value graphMain" style="width:60%"><xsl:apply-templates select="rcwar:StayActivityDateDetails[1]"/></td>
						</tr>
						<xsl:for-each select="rcwar:StayActivityDateDetails[position() &gt; 1]">
							<tr>
	...
	<xsl:template match="rcwar:RegisterDocumentDetails">
		<table class="w190">
			<tr>
	...
	<xsl:template match="rcwar:RegisterOrganizationDetails">
	...
	<xsl:template match="rcwar:ResourceItemStatusDetails">
	...
	<!-- Шаблон для типа rcwar:StayActivityDateType -->
	<xsl:template match="rcwar:StayActivityDateDetails">
		<xsl:text>с </xsl:text>
		<xsl:call-template name="russian_date">
			<xsl:with-param name="dateIn" select="rcwar:LastDayActivity"/>
		</xsl:call-template>
		<xsl:if test="rcwar:FirstDayActivity">
			<xsl:text> по </xsl:text>
			<xsl:call-template name="russian_date">
				<xsl:with-param name="dateIn" select="rcwar:FirstDayActivity"/>
			</xsl:call-template>
		</xsl:if>
	</xsl:template>
