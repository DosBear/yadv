...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:clt_ru="urn:customs.ru:CommonLeafTypes:5.10.0" xmlns:cat_ru="urn:customs.ru:CommonAggregateTypes:5.10.0" xmlns:RUSclt_ru="urn:customs.ru:RUSCommonLeafTypes:5.13.3" xmlns:RUScat_ru="urn:customs.ru:RUSCommonAggregateTypes:5.13.3" xmlns:RUDECLcat="urn:customs.ru:RUDeclCommonAggregateTypesCust:5.13.3" xmlns:rccar="urn:customs.ru:Information:SQDocuments:RegisterCustCarrier:5.13.3">
		...
		<html>
			...
			<body>
				<div class="page">
						...
						<tbody>
							<tr>
								<td align="center" class="graphMain">
									<font size="4">
										<b>Реестр таможенных перевозчиков</b>
									</font>
								</td>
							</tr>
						</tbody>
					...
					<xsl:if test="rccar:EECEDocHeaderAddInfo">
						<div class="title marg-t1">Дополнительные технологические реквизиты электронного документа (сведений)</div>
						<xsl:apply-templates select="rccar:EECEDocHeaderAddInfo"/>
					</xsl:if>
					<div class="title marg-t1">Сведения об объекте реестра таможенных перевозчиков</div>
					<table class="bordered w190">
						<xsl:for-each select="rccar:RegisterCustCarrierDetails">
	...
	<xsl:template match="rccar:EECEDocHeaderAddInfo">
	...
	<xsl:template match="rccar:GuaranteeDocDetails">
		<tr>
	...
	<!-- Шаблон для типа rccar:RegisterCustCarrierDetailsType -->
	<xsl:template match="rccar:RegisterCustCarrierDetails">
		<tr>
			<td class="annot graphMain">
				<table class="w190">
					<tr>
						<td class="annot graphMain" style="width:40%">Кодовое обозначение страны, предоставившей информацию</td>
						<td class="value graphMain" style="width:60%"><xsl:value-of select="rccar:RegisterCountryCode"/></td>
					</tr>
					<xsl:if test="rccar:CustomsOfficeCode">
						<tr>
							<td class="annot graphMain" style="width:40%">Код таможенного органа</td>
							<td class="value graphMain" style="width:60%"><xsl:value-of select="rccar:CustomsOfficeCode"/></td>
						</tr>
					</xsl:if>
					<xsl:apply-templates select="rccar:ResourceItemStatusDetails"/>
				</table>
				<div class="title marg-top">Сведения о документе, подтверждающем включение юридического лица в реестр</div>
				<xsl:apply-templates select="rccar:RegisterDocumentDetails"/>
				<div class="title marg-top">Сведения о реквизитах юридического лица, включенного в реестр</div>
				<xsl:apply-templates select="rccar:RegisterOrganizationDetails"/>
				<div class="title marg-top">Сведения о документе, подтверждающем обеспечение уплаты таможенных пошлин, налогов</div>
				<table class="bordered w190">
					<tr class="title">
						<td class="graphMain bordered">Наименование документа</td>
						<td class="graphMain bordered">Период, в течение которого документ имеет силу</td>
						<td class="graphMain bordered">Код страны</td>
					</tr>
					<xsl:for-each select="rccar:GuaranteeDocDetails">
						<xsl:apply-templates select="."/>
					</xsl:for-each>
				</table>
				<xsl:if test="rccar:AdditionalInfoText">
					<div class="title marg-top">Дополнительная информация, относящаяся к описываемому объекту</div>
					<xsl:value-of select="rccar:AdditionalInfoText"/>
				</xsl:if>
				<div class="title marg-top">Период осуществления деятельности</div>
				<table class="w190">
					<tr>
						<td class="annot graphMain" style="width:40%">1-й день действия, в том числе после временного прекращения / приостановления</td>
						<td class="value graphMain" style="width:60%">
							<xsl:call-template name="russian_date">
								<xsl:with-param name="dateIn" select="rccar:StartActivityDate"/>
							</xsl:call-template>
						</td>
					</tr>
					<xsl:if test="rccar:EndActivityDate">
						<tr>
							<td class="annot graphMain" style="width:40%">последний день действия, в том числе по причине временного прекращения</td>
							<td class="value graphMain" style="width:60%">
								<xsl:call-template name="russian_date">
									<xsl:with-param name="dateIn" select="rccar:EndActivityDate"/>
								</xsl:call-template>
							</td>
						</tr>
					</xsl:if>
	...
	<xsl:template match="rccar:RegisterDocumentDetails">
		<table class="w190">
			<tr>
	...
	<xsl:template match="rccar:RegisterOrganizationDetails">
	...
	<xsl:template match="rccar:ResourceItemStatusDetails">
