...
<xsl:stylesheet version="1.0" xmlns:cat_ru="urn:customs.ru:CommonAggregateTypes:5.10.0" xmlns:poisub="urn:customs.ru:Information:SQDocuments:RosPrirodPoisonousSubstances:5.13.3" xmlns:sq_cat="urn:customs.ru:Information:SQDocuments:SQCommonAgregateTypesCust:5.10.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:RUScat_ru="urn:customs.ru:RUSCommonAggregateTypes:5.13.3">
   ...
   <xsl:template match="poisub:RosPrirodPoisonousSubstances">
      <html>
         <head>
            <title>Заключение уполномоченного органа государств - членов Евразийского экономического союза на ввоз (вывоз) ядовитых веществ, не являющихся прекурсорами наркотических средств и психотропных веществ и являющихся стандартными образцами</title>
         ...
         <body>
            <div class="page">
               <h2>Заключение уполномоченного органа государств - членов Евразийского экономического союза на ввоз (вывоз) ядовитых веществ, не являющихся прекурсорами наркотических средств и психотропных веществ и являющихся стандартными образцами</h2>
               <p class="remark">
                  <xsl:choose>
                     <xsl:when test="poisub:IsCanceled='true' or poisub:IsCanceled='t' or poisub:IsCanceled='1' ">Документ не действителен</xsl:when>
               ...
               <table border="0">
                  <tbody>
                     <tr style="vertical-align:top">
                        ...
                        <td>
                           <xsl:value-of select="poisub:DocumentId"/>
                        </td>
                     </tr>
                     <xsl:if test="poisub:BDRDID">
						 <tr style="vertical-align:top">
							<td width="30%">Id док-та по БД РД</td>
							<td>
							   <xsl:value-of select="poisub:BDRDID"/>
							</td>
						 </tr>
                     </xsl:if>
                     <tr style="vertical-align:top">
                        <td width="30%">Описание ядовитых веществ</td>
                        <td>
                           <xsl:value-of select="poisub:Info"/>
                        </td>
                     </tr>
                     <tr>
                        <td>Код ТН ВЭД ЕАЭС</td>
                        <td>
                           <xsl:value-of select="poisub:TNVED"/>
                        </td>
                     </tr>
                     <tr>
                        <td>Количество</td>
                        <td>
                           <xsl:apply-templates select="poisub:Count"/>
                        </td>
                     </tr>
                     <tr>
                        <td>Заключение</td>
                        <td>
                           <xsl:apply-templates select="poisub:Conclusion"/>
                        </td>
                     </tr>
                     <xsl:if test="poisub:AuthorizedPerson">
						 <tr>
							<td>Уполномоченное должностное лицо, подписавшее заключение</td>
							<td>
							   <xsl:apply-templates select="poisub:AuthorizedPerson"/>
							</td>
						 </tr>
                     </xsl:if>
                     <tr>
                        <td>Отправитель</td>
                        <td>
                           <xsl:apply-templates select="poisub:Sender"/>
                        </td>
                     </tr>
                     <tr>
                        <td>Получатель</td>
                        <td>
                           <xsl:apply-templates select="poisub:Receiver"/>
                        </td>
                     </tr>
                     <tr>
                        <td>Контракт</td>
                        <td>
                           <xsl:apply-templates select="poisub:Contract"/>
                        </td>
                     </tr>
                  </tbody>
               </table>
   ...
   <xsl:template match="poisub:Conclusion | poisub:Contract">
      <span>
   ...
   <xsl:template match="poisub:Sender | poisub:Receiver">
   ...
   <xsl:template match="poisub:Count">
