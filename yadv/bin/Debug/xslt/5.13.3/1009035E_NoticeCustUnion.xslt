...
<xsl:stylesheet version="1.0" xmlns:cat_ru="urn:customs.ru:CommonAggregateTypes:5.10.0" xmlns:RUScat_ru="urn:customs.ru:RUSCommonAggregateTypes:5.13.3" xmlns:ncuni="urn:customs.ru:Information:SQDocuments:NoticeCustUnion:5.13.3" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
   ...
   <xsl:template match="ncuni:NoticeCustUnion">
      <html>
         <head>
         ...
         <body>
            <div class="page">
               ...
               <p class="remark">
                  <xsl:choose>
                     <xsl:when test="ncuni:IsPerformed = 'true' or ncuni:IsPerformed = 't' or ncuni:IsPerformed = '1' ">
               ...
               <span>
                  <span class="header">Идентификатор записи:  </span>
                  <xsl:value-of select="ncuni:UniNotificationID"/>
               </span>
               <br/>
               <br/>
               <table border="1">
                  <tbody>
                     <tr>
                        <th>Контрольный пункт для представления недостающих документов</th>
                        <th>Контрольный пункт, на котором установили отметку</th>
                     </tr>
                     <tr>
                        <td>
							Наименование: <xsl:value-of select="ncuni:DestOrganizationName"/>
							<br/>
							Местонахождение: <xsl:value-of select="ncuni:DestRegionCode"/>
                        </td>
                        <td>
							Наименование: <xsl:value-of select="ncuni:PerformedOrganization"/>
							<br/>
							Ф.И.О. инспектора: <xsl:value-of select="ncuni:PerformedEmployee/cat_ru:PersonName"/>
							<xsl:if test="ncuni:PerformedEmployee/cat_ru:LNP">
								&#160;ЛНП: <xsl:value-of select="ncuni:PerformedEmployee/cat_ru:LNP"/>
							</xsl:if>
                        </td>
                     </tr>
                     <tr>
                        <th>Дата/время проведения контроля</th>
                        <th>Дата/время установки отметки</th>
                     </tr>
                     <tr>
                        <td>
                           <xsl:call-template name="date_time">
                              <xsl:with-param name="dateTimeIn" select="ncuni:InspectionDateTime"/>
                           </xsl:call-template>
                        </td>
                        <td>
                           <xsl:call-template name="date_time">
                              <xsl:with-param name="dateTimeIn" select="ncuni:PerformedDate"/>
                           </xsl:call-template>
                        </td>
                     </tr>
                     <tr>
                        <th>Cпециальные разрешения</th>
                        <th>Уведомление</th>
                     </tr>
                     <tr>
                        <td>
                           <xsl:if test="ncuni:PassTGNumber">Тяжеловесные грузы: <xsl:value-of select="ncuni:PassTGNumber"/>
                              <br/>
                           </xsl:if>
                           <xsl:if test="ncuni:PassKGNumber">Крупногабаритные грузы: <xsl:value-of select="ncuni:PassKGNumber"/>
                              <br/>
                           </xsl:if>
                           <xsl:if test="ncuni:PassOGNumber">Опасные грузы: <xsl:value-of select="ncuni:PassOGNumber"/>
                           </xsl:if>
                        </td>
                        <td>
                           <xsl:apply-templates select="ncuni:Notification"/>
                        </td>
                     </tr>
                     <tr>
                        <th colspan="2">Организация, проводившая контроль</th>
                     </tr>
                     <tr>
                        <td colspan="2">
                           <xsl:apply-templates select="ncuni:NotificationOrg"/>
                        </td>
                     </tr>
                     <tr>
                        <th>Водитель</th>
                     ...
                     <tr>
                        <td>
                           <xsl:apply-templates select="ncuni:Driver"/>
                        </td>
                        <td>
                           <xsl:apply-templates select="ncuni:Carrier"/>
                        </td>
                     </tr>
                     <tr>
                        <th>Разрешение на поездку</th>
                        <th>Выявленные нарушения</th>
                     </tr>
                     <tr>
                        <td>
                           <xsl:apply-templates select="ncuni:TransitPermit"/>
                        </td>
                        <td>
                           <xsl:apply-templates select="ncuni:Violation"/>
                        </td>
                     </tr>
                     <tr>
                        <th colspan="2">Транспортное средство</th>
                     </tr>
                     <tr>
                        <td colspan="2">
                           <xsl:apply-templates select="ncuni:TSInfo"/>
                        </td>
                     </tr>
                  </tbody>
               </table>
   ...
   <xsl:template match="ncuni:NotificationOrg">
      <xsl:choose>
         <xsl:when test="cat_ru:OfficeName">
            <xsl:value-of select="cat_ru:OfficeName"/> 
			(код ТО - <xsl:value-of select="cat_ru:Code"/>; 
			<xsl:if test="cat_ru:CustomsCountryCode">код страны - <xsl:value-of select="cat_ru:CustomsCountryCode"/>
            </xsl:if>)
		</xsl:when>
         <xsl:otherwise>
			Код ТО - <xsl:value-of select="cat_ru:Code"/>
            <xsl:if test="cat_ru:CustomsCountryCode">(код страны - <xsl:value-of select="cat_ru:CustomsCountryCode"/>)</xsl:if>
         </xsl:otherwise>
      </xsl:choose>
   </xsl:template>
   <xsl:template match="ncuni:Driver">
   ...
   <xsl:template match="ncuni:Carrier">
      ...
      <xsl:if test="cat_ru:RFOrganizationFeatures or cat_ru:RKOrganizationFeatures or cat_ru:RBOrganizationFeatures or cat_ru:RAOrganizationFeatures or cat_ru:KGOrganizationFeatures">
		...
		<xsl:if test="RUScat_ru:CountryA2Code">
			Кодовое обозначение страны: <xsl:value-of select="RUScat_ru:CountryA2Code"/>
			<br/>
		</xsl:if>
		...
		<xsl:if test="RUScat_ru:PersonId">
			Уникальный идентификатор физического лица: <xsl:value-of select="RUScat_ru:PersonId"/>
			<br/>
		</xsl:if>
		<xsl:if test="RUScat_ru:IdentityCard">
			Сведения о док-те, удостоверяющем личность физ. лица: <br/>
   ...
   <xsl:template match="ncuni:TransitPermit | ncuni:Notification">
      <span>
   ...
   <xsl:template match="ncuni:Violation">
      <span>
		Нарушение № 
		<xsl:value-of select="ncuni:NodeNumber"/>: 
		ID <xsl:value-of select="ncuni:ViolationID"/>&#160;
		<xsl:value-of select="ncuni:ViolationName"/>, 
		(<xsl:value-of select="ncuni:ControlStatusID"/>) 
		<xsl:value-of select="ncuni:ControlStatusName"/>
      </span>
      <br/>
   </xsl:template>
   <xsl:template match="ncuni:TSInfo">
