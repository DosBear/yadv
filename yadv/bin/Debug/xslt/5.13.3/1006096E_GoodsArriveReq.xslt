...
<xsl:stylesheet exclude-result-prefixes="gar cat_ru catESAD_cu" version="1.0" xmlns:catESAD_cu="urn:customs.ru:CUESADCommonAggregateTypesCust:5.12.0" xmlns:cat_ru="urn:customs.ru:CommonAggregateTypes:5.10.0" xmlns:gar="urn:customs.ru:Information:CustomsDocuments:GoodsArriveReq:5.12.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
   ...
   <xsl:template match="gar:GoodsArriveReq">
      <html>
         <head>
            <title>Запрос о прибытии товарной партии</title>
         ...
         <body>
            <div class="page">
               <h2>
						Запрос о прибытии товарной партии <br/>
                  <xsl:if test="gar:ReqNumber">№
							<xsl:value-of select="gar:ReqNumber"/>
                     <xsl:text disable-output-escaping="yes">&amp;nbsp;&amp;nbsp;&amp;nbsp;</xsl:text>
                  </xsl:if>
							от <xsl:call-template name="russian_date">
                     <xsl:with-param name="dateIn" select="gar:ReqDate"/>
                  </xsl:call-template>
               </h2>
               <table border="0" cellspacing="0" width="100%">
                  <tbody>
                     <xsl:if test="gar:GTDID">
                        <tr>
                           <td class="graphNo" style="width:90mm;">
									Регистрационнный номер ГТД:
								</td>
                           <td class="graph" style="width:90mm;">
                              <xsl:value-of select="gar:GTDID/cat_ru:CustomsCode"/>
                              <xsl:text> / </xsl:text>
                              <xsl:call-template name="russian_date_gtd">
                                 <xsl:with-param name="dateIn" select="gar:GTDID/cat_ru:RegistrationDate"/>
                              </xsl:call-template>
                              <xsl:text> / </xsl:text>
                              <xsl:value-of select="gar:GTDID/cat_ru:GTDNumber"/>
                           </td>
                        </tr>
                     </xsl:if>
                     <tr>
                        <td class="graph" colspan="2" style="width:180mm;">
                           ...
                           <xsl:apply-templates select="gar:Consignee"/>
                        </td>
                     </tr>
                     <xsl:if test="gar:Consignor">
                        <tr>
                           <td class="graph" colspan="2" style="width:180mm;">
                              <span class="graphNo">Грузоотправитель </span>
                              <xsl:apply-templates select="gar:Consignor"/>
                           </td>
                        </tr>
                     </xsl:if>
                     <xsl:if test="gar:GoodsShipment/gar:ContainerNumber">
                        <tr>
                           <td class="graph" colspan="2" style="width:180mm;">
                              <span class="graphNo"> Контейнеры: </span>
                              <xsl:for-each select="gar:GoodsShipment/gar:ContainerNumber">
                     ...
                     <tr>
                        <td class="graph" colspan="2" style="width:180mm;">
                           <span class="graphNo">Транспортные и перевозочные документы</span>
                           <br/>
                           <table border="1" cellpadding="6" cellspacing="0" width="100%">
                              <tbody>
                                 ...
                                 <xsl:for-each select="gar:GoodsShipment/gar:PresentDocument">
                                    <tr>
                                       ...
                                       <td align="center" class="graph" style="width:30mm;">
                                          <xsl:value-of select="gar:PresentedDocumentModeCode"/>
                                       </td>
                                    </tr>
                                 </xsl:for-each>
                              </tbody>
                     ...
                     <tr>
                        ...
                        <td class="graph" style="width:90mm;">
                           <xsl:value-of select="gar:Customs/cat_ru:Code"/>
                           <xsl:text disable-output-escaping="yes">&amp;nbsp;</xsl:text>
                           <xsl:if test="gar:Customs/cat_ru:OfficeName">
                              <xsl:value-of select="gar:Customs/cat_ru:OfficeName"/>
                           </xsl:if>
                        </td>
                     </tr>
                     <tr>
                        ...
                        <td class="graph" style="width:90mm;">
                           <xsl:value-of select="gar:CustomsPerson/cat_ru:PersonName"/>
                           <xsl:text disable-output-escaping="yes">&amp;nbsp;</xsl:text>
                           <xsl:if test="gar:CustomsPerson/cat_ru:LNP">
											ЛНП <xsl:text disable-output-escaping="yes">&amp;nbsp;</xsl:text>
                              <xsl:value-of select="gar:CustomsPerson/cat_ru:LNP"/>
                           </xsl:if>
                        </td>
                     </tr>
                  </tbody>
   ...
   <xsl:template match="gar:Consignee | gar:Consignor ">
