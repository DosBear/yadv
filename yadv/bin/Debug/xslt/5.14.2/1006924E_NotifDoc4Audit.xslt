...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:clt_ru="urn:customs.ru:CommonLeafTypes:5.10.0" xmlns:cat_ru="urn:customs.ru:CommonAggregateTypes:5.10.0" xmlns:nda="urn:customs.ru:Information:CustomsAuditDocuments:NotifDoc4Audit:5.14.2" xmlns:adt_cat="urn:customs.ru:Information:CustomsAuditDocuments:AuditCommonAggregateTypesCust:5.14.0" xmlns:RUScat_ru="urn:customs.ru:RUSCommonAggregateTypes:5.14.0">
	<!-- 
Приложение (ГУТКПВТ 53522 Формы документов по ФЗ-289 (приложение).docx, форма 6) к Письму ГУТКПВТ от 24.09.2018 № 23-20/53522
-->
	<!--ИМО-000569 Письмо ФТС России от 27.09.2018 № 01-54/61519, Проект приказа ФТС России "Об утверждении форм документов, составляемых таможенными органами при проведении таможенных проверок,порядков их заполнения и порядка внесения изменений в решение о проведении выездной таможенной проверки" (Приложение 21)
-->
						...
						<tbody align="left" class="graphMain">
							...
							<tr align="center">
								...
								<td class="value" width="50%">
									<xsl:if test="nda:AuditOrganization/cat_ru:OrganizationName">
										<xsl:value-of select="nda:AuditOrganization/cat_ru:OrganizationName"/>
									</xsl:if>
									<xsl:if test="nda:AuditOrganization/cat_ru:ShortName">
										<xsl:if test="string-length(nda:AuditOrganization/cat_ru:OrganizationName) &gt; 0">
											<span class="normal"> (</span>
										</xsl:if>
										<xsl:value-of select="nda:AuditOrganization/cat_ru:ShortName"/>
										<xsl:if test="string-length(nda:AuditOrganization/cat_ru:OrganizationName) &gt; 0">
											<span class="normal">)</span>
										</xsl:if>
									</xsl:if>
							...
							<tr align="center">
								<td/>
								<td class="graphLittle">(наименование юридического лица, фамилия и инициалы отчества (инициал отчества указывается при наличии отчества) физического лица, в том числе индивидуального предпринимателя)</td>
							</tr>
							<tr align="center">
								<td/>
								<td class="value">
									<xsl:if test="nda:AuditOrganization/RUScat_ru:SubjectAddressDetails">
										<xsl:for-each select="nda:AuditOrganization/RUScat_ru:SubjectAddressDetails">
									...
									<xsl:if test="string-length(nda:AuditOrganization/RUScat_ru:SubjectAddressDetails) = '0'">
										<xsl:for-each select="nda:AuditOrganization/nda:FactAddressDetails">
											<xsl:for-each select="RUScat_ru:PostalCode|RUScat_ru:CountryCode|RUScat_ru:CounryName|RUScat_ru:Region|RUScat_ru:District|RUScat_ru:Town|RUScat_ru:City|RUScat_ru:StreetHouse|RUScat_ru:House|RUScat_ru:Room|RUScat_ru:AddressText">
												<xsl:value-of select="."/>
												<xsl:if test="position()!=last()">, </xsl:if>
											</xsl:for-each>
	...
	<xsl:template match="nda:AuditOrganization | nda:ConnectedOrganization">
