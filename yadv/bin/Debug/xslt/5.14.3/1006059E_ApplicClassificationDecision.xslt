					...
					<xsl:if test="acd:IDDoc='КР'">
						<table border="0" style="width:190mm">
							<tbody>
								...
								<xsl:if test="acd:FormDecision">
									<tr>
										<td>
											<b>Вид формы принятия решения</b><br/>
											<xsl:choose>
												...
												<xsl:otherwise><xsl:value-of select="acd:FormDecision"/></xsl:otherwise>
											</xsl:choose>
										</td>
									</tr>
								</xsl:if>
								<xsl:if test="acd:ReasonDecision">
									<tr>
										<td>
											<b>Основание для принятия решения о внесении изменений в решение о классификации товара</b><br/>
											<xsl:for-each select="acd:ReasonDecision">
												<xsl:if test="acd:ReasonDecisionKind">
													предусмотренное частью 2 статьи 17 Федерального закона от 3 августа 2018 г. № 289-ФЗ:<br/>
												...
												<xsl:if test="acd:ReasonDecisionKind and acd:ReasonDecisionText">.<br/></xsl:if>
												<xsl:if test="acd:ReasonDecisionText">
								...
								<xsl:if test="acd:OtherInfo">
									<tr>
										<td>
											<b>Иные сведения, которые, по мнению заявителя, необходимо сообщить</b><br/>
