...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:catESAD_cu="urn:customs.ru:CUESADCommonAggregateTypesCust:5.12.0" xmlns:cat_ru="urn:customs.ru:CommonAggregateTypes:5.10.0" xmlns:pgc="urn:customs.ru:Information:CustomsDocuments:PaymentGuaranteeCalc:5.13.2" xmlns:RUSclt_ru="urn:customs.ru:RUSCommonLeafTypes:5.13.0" xmlns:RUScat_ru="urn:customs.ru:RUSCommonAggregateTypes:5.13.0" xmlns:RUDECLcat="urn:customs.ru:RUDeclCommonAggregateTypesCust:5.13.2" xmlns:cltESAD_cu="urn:customs.ru:CUESADCommonLeafTypes:5.12.0" xmlns:clt_ru="urn:customs.ru:CommonLeafTypes:5.10.0" xmlns:xs="http://www.w3.org/2001/XMLSchema">
						...
						<xsl:for-each select="pgc:GoodsItemPaymentDetails">
							<tr valign="top">
								<td class="bordered" style="border-left:0;border-bottom:0;">
									<xsl:value-of select="pgc:PaymentModeCode"/>
								</td>
								<td class="bordered" style="border-bottom:0;word-break:break-all">
									<xsl:value-of select="pgc:TaxBase"/>
									<xsl:text> </xsl:text>
									<xsl:choose>
										<xsl:when test="pgc:TaxBaseCurrencyCode"><xsl:apply-templates select="pgc:TaxBaseCurrencyCode"/></xsl:when>
										<xsl:when test="pgc:TaxBaseQualifierCode"><xsl:value-of select="pgc:TaxBaseQualifierCode"/></xsl:when>
									</xsl:choose>
								</td>
								...
								<td class="bordered" style="border-right:0;border-bottom:0;">
									<xsl:value-of select="pgc:PaymentCode"/>
								</td>
							</tr>
						</xsl:for-each>
					</table>
	...
	<xsl:template match="*" mode="rate1">
		<xsl:variable name="rateval">
			<xsl:choose>
				<xsl:when test="pgc:Rate or pgc:Rate!=''">
					<xsl:value-of select="pgc:Rate"/>
				</xsl:when>
				<xsl:otherwise>0</xsl:otherwise>
			</xsl:choose>
		</xsl:variable>
		...
		<xsl:choose>
			<xsl:when test="pgc:RateTypeCode='%'">%</xsl:when>
			<xsl:when test="pgc:RateTypeCode!='%' or not(pgc:RateTypeCode)">
				<xsl:choose>
					<xsl:when test="pgc:RateCurrencyCode">
						<xsl:text> </xsl:text>
						<xsl:apply-templates select="pgc:RateCurrencyCode"/>
						<xsl:if test="pgc:RateTNVEDQualifierCode">
							<xsl:text> за </xsl:text>
							<xsl:value-of select="pgc:WeightingFactor"/>
							<xsl:text> </xsl:text>
							<xsl:apply-templates select="pgc:RateTNVEDQualifierCode"/>
						</xsl:if>
					</xsl:when>
	...
	<xsl:template match="pgc:PaymentCurrencyCode | pgc:PreviousPaymentCurrencyCode | pgc:DifferencePaymentCurrencyCode | pgc:TaxBaseCurrencyCode | pgc:RateCurrencyCode | RUScat_ru:CurrencyCode">
		<xsl:choose>
	...
	<xsl:template match="pgc:RateTNVEDQualifierCode">
		<xsl:choose>
