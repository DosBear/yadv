...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:cat_ru="urn:customs.ru:CommonAggregateTypes:5.10.0" xmlns:adt_cat="urn:customs.ru:Information:CustomsAuditDocuments:AuditCommonAggregateTypesCust:5.13.2" xmlns:dca="urn:customs.ru:Information:CustomsAuditDocuments:DecisionCustomsAudit:5.13.2" xmlns:RUScat_ru="urn:customs.ru:RUSCommonAggregateTypes:5.13.0" xmlns:catESAD_cu="urn:customs.ru:CUESADCommonAggregateTypesCust:5.12.0">
	...
	<xsl:template match="dca:DecisionCustomsAudit">
		<html>
			...
			<body>
				<div class="page">
					<xsl:choose>
						<xsl:when test="not(dca:PrevDecisionNumber)"><xsl:apply-templates select="." mode="decision"/></xsl:when>
						<xsl:otherwise><xsl:apply-templates select="." mode="decisionChange"/></xsl:otherwise>
					</xsl:choose>
				</div>
			</body>
		</html>
	...
	<xsl:template match="dca:DecisionCustomsAudit" mode="decision">
			...
			<tbody>
				...
				<tr>
					<td align="center">
						...
						<br/>
						№ <span style="border-bottom:1px solid black; width:50mm;"><xsl:value-of select="dca:DecisionNumber/adt_cat:Number"/></span>
						<br/>
						<xsl:call-template name="russian_date_month"><xsl:with-param name="dateIn" select="dca:DecisionNumber/adt_cat:Date"/></xsl:call-template> г.
					</td>
				</tr>
				...
				<tr>
					<td class="indent">
						<xsl:text>1.2. Лицо, в отношении товаров, документов, сооружений, помещений (частей помещений), открытых площадок (частей открытых площадок) и (или) деятельности которого проведена таможенная проверка / проверка таможенных, иных документов и (или) сведений:</xsl:text>
					</td>
				</tr>
				<tr>
					<td class="value">
				...
				<tr>
					<td class="indent">
						<xsl:text>2.1. На основании акта таможенной проверки / проверки таможенных, иных документов и (или) сведений от </xsl:text>
						...
						<xsl:text> № </xsl:text>
						<xsl:call-template name="inlinetable">
							<xsl:with-param name="data"><xsl:value-of select="dca:ActNumber/adt_cat:Number"/></xsl:with-param>
				...
				<tr>
					<td>
						<xsl:call-template name="inlinetable">
							<xsl:with-param name="data"><xsl:apply-templates select="dca:AuditDisorders"/></xsl:with-param>
				...
				<tr>
					<td class="indent">
						<xsl:text>3.1. На основании изложенного, руководствуясь статьей 218 и частью 2 статьи 226 / частью 6 статьи 237 Федерального закона от ___________ № _____  «О таможенном регулировании», РЕШЕНО:</xsl:text>
					</td>
				</tr>
				<tr>
					<td class="indent">
									...
									<tr valign="top">
										...
										<td class="bordered">
											<xsl:choose>
												...
												<xsl:when test="dca:Decision_Variant_4='true'">Решение о признании товаров товарами, подлежащими изъятию</xsl:when>
												<xsl:when test="dca:Decision_Variant_5='true'">Решение о признании лица, участвовавшего в незаконном перемещении товаров через   таможенную границу Евразийского экономического союза, несущим солидарную  обязанность по уплате таможенных платежей, специальных, антидемпинговых, компенсационных пошлин, процентов и пени</xsl:when>
												<xsl:when test="dca:Decision_Variant_6='true'">Решение о признании лица, которое приобрело в собственность или во владение незаконно ввезенные товары, несущим солидарную  обязанность по уплате таможенных платежей, специальных, антидемпинговых, компенсационных пошлин, процентов и пени</xsl:when>
											</xsl:choose>
											<xsl:text>: </xsl:text>
											<br/>
											...
											<xsl:if test="dca:PaymentsInfo">
												...
												<xsl:for-each select="dca:PaymentsInfo">
													...
													<xsl:text>/</xsl:text>
													<xsl:call-template name="russian_date"><xsl:with-param name="dateIn" select="dca:PaymentTermDate"/></xsl:call-template>
													<xsl:text>/</xsl:text>
													<xsl:call-template name="number_date"><xsl:with-param name="dateIn" select="dca:CalculationRatesDate"/></xsl:call-template>
												...
												<table>
													<tbody>
														<tr>
															...
															<td style="width:50%;border-right:1px solid black;"><xsl:value-of select="dca:OriginCountryCode"/></td>
								...
								<tbody>
									...
									<tr valign="top">
										...
										<td class="bordered" style="border-top:0;">дата
											<br/>
											<xsl:call-template name="russian_date"><xsl:with-param name="dateIn" select="dca:PassengerDT/adt_cat:Date"/></xsl:call-template>
										</td>
									</tr>
									<tr valign="top">
		...
		<xsl:call-template name="inlinetable">
			<xsl:with-param name="data">
				<xsl:call-template name="russian_date_month"><xsl:with-param name="dateIn" select="dca:SecondExempReceived/adt_cat:Date"/></xsl:call-template>
			</xsl:with-param>
			...
			<tbody>
				<tr valign="top">
					<td width="40%" class="value">
						<xsl:call-template name="russian_date"><xsl:with-param name="dateIn" select="dca:DirectionByPost/adt_cat:Date"/></xsl:call-template>
					</td>
					<td width="10%"></td>
					<td width="50%" class="value">
	...
	<xsl:template mode="footer" match="*">
		<xsl:choose>
			<xsl:when test="dca:AuditOrganization/RUScat_ru:IdentityCard and not(dca:AuditOrganization/cat_ru:RFOrganizationFeatures/cat_ru:KPP)">
				...
				<xsl:call-template name="inlinetable">
					<xsl:with-param name="data">
						<xsl:apply-templates mode="address" select="dca:Appeals/dca:CustomsSuperior/dca:Address"/>
					</xsl:with-param>
	...
	<xsl:template name="inlinetable">
		...
		<table style="display:inline-table;width:{$width};text-indent:0;">
			<tbody>
				<tr>
					<td class="value">
