	...
	<xsl:template match="culc:CultureCertificate">
		<html>
			...
			<table align="center" style="width:170mm">
				...
				<tr>
					<td align="center" class="graph12">Выдано <u>
							<xsl:apply-templates select="culc:OrganizationIssued/RUScat_ru:OrganizationName"/>
						</u>
					</td>
				</tr>
				<tr>
			...
			<xsl:if test="culc:NumSheets">
				<table>
					<tr>
						<td class="graph10">
							Список предметов на <u> <xsl:value-of select="culc:NumSheets"/> </u> листах прилагается
						</td>
					</tr>
				</table>	
			</xsl:if>
					...
					<tr>
						<td class="graph10">
							...
							<xsl:if test="culc:NumFoto">
								...
								<u> <xsl:value-of select="culc:NumFoto"/> </u>
								 шт. прилагаются
							</xsl:if>
						</td>
					</tr>
			...
			<table>
				<tr>
					...
					<td align="center" class="underlined" style="width:40%">
						<span class="graph10"><xsl:value-of select="culc:CountryName"/></span>
					</td>
					<td style="width:30%"/>
				</tr>	
				<tr>
				...
				<table style="width: 100%">
					<tr>
						...
						<td class="underlined" style="width: 50%">
							<span class="graph10"><xsl:value-of select="culc:Target"/></span>
						</td>
						<td align="right" class="graph10" style="width: 15%">срок вывоза </td>
						<td align="center" class="underlined" style="width: 20%">
							<span class="graph10">
			...
			<table align="center" style="width:100%">
				<tr>
					...
					<td align="center" class="underlined" style="width:65%">
						...
						<span class="graph10"> выданного </span>
						<xsl:value-of select="culc:DecisionInf/culc:StateStructure"/>
					</td>
				</tr>
				<tr>
					<td/>
					...
					<td align="center" class="graph10">от  <u>
						<xsl:choose>
							<xsl:when test="culc:DecisionInf/cat_ru:PrDocumentDate">
								<xsl:for-each select="culc:DecisionInf/cat_ru:PrDocumentDate"><xsl:call-template name="Date"/> </xsl:for-each>
							</xsl:when>
							<xsl:otherwise>
								<xsl:for-each select="culc:DateIssued"><xsl:call-template name="Date"/> </xsl:for-each>
							</xsl:otherwise>
						</xsl:choose>
						</u>
					№ <u>
			...
			<table border="1" cellpadding="1" cellspacing="0" style="width:100%" align="center">
				<tbody>
					<tr>
						...
						<xsl:if test="//culc:Description"><td align="center" style="width:40%">Описание</td></xsl:if>
						<td align="center" style="width:20%">Количество</td>
					</tr>
					<xsl:for-each select="culc:CulturalValuesList">
						<tr>
							<td align="center" class="graph10">
								<xsl:for-each select="culc:CulturalName"><xsl:value-of select="."/></xsl:for-each>
							</td>
							<xsl:if test="//culc:Description">
								<td align="center" class="graph10" style="width:40%">
									<xsl:for-each select="culc:Description"><xsl:value-of select="."/></xsl:for-each>
								</td>
							</xsl:if>	
					...
					<table align="center" border="1" cellpadding="1" cellspacing="0" class="graph10" style="width: 100%">
						<xsl:if test="culc:BDRDID">
							<tr>
								...
								<td style="width: 60%"><xsl:value-of select="culc:BDRDID"/></td>
							</tr>	
						</xsl:if>
							...
							<xsl:for-each select="culc:OrganizationIssued">
								<tr>
									...
									<td style="width: 60%; border">
										<xsl:for-each select="RUScat_ru:OrganizationName"><xsl:value-of select="."/></xsl:for-each>
										...
										<xsl:if test="RUScat_ru:Address"> Юридический адрес: <xsl:value-of select="RUScat_ru:Address"/></xsl:if>
									</td>
								</tr>
							</xsl:for-each>
						</xsl:if>	
						<xsl:if test="culc:OrgUnitCode">
							<tr>
								...
								<td style="width: 60%"><xsl:value-of select="culc:OrgUnitCode"/></td>
							</tr>	
						</xsl:if>
					</table>
				</div>
