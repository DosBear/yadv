...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:catESAD_cu="urn:customs.ru:CUESADCommonAggregateTypesCust:5.12.0" xmlns:cat_ru="urn:customs.ru:CommonAggregateTypes:5.10.0" xmlns:dcg="urn:customs.ru:Information:CustomsDocuments:DecisionClassGood:5.13.1">
					...
					<table class="main" border="1" style="border:0; border-collapse:collapse; margin: -5px; width:100%">
						...
						<tr>
							<td colspan="2">
								...
								9.1.&#160;Товар №<u>
										<xsl:choose>
											<xsl:when test="dcg:Goods/dcg:Numeric_TNVEDCode/dcg:GoodsNumeric and (dcg:Goods/dcg:IsPartialGood='0' or dcg:Goods/dcg:IsPartialGood='f' or dcg:Goods/dcg:IsPartialGood='false')">
											<xsl:for-each select="dcg:Goods/dcg:Numeric_TNVEDCode">
												<xsl:value-of select="dcg:GoodsNumeric"/>
												<xsl:if test="position()!=last()">, </xsl:if>
												</xsl:for-each>
											</xsl:when>
											<xsl:when test="dcg:Goods/dcg:IsPartialGood='1' or dcg:Goods/dcg:IsPartialGood='t' or dcg:Goods/dcg:IsPartialGood='true'"> часть товара
								...
								<div class="content">
								9.8.&#160;Прочие отметки <xsl:for-each select="dcg:Mark">
										<xsl:value-of select="."/>
										<xsl:text> </xsl:text>
									</xsl:for-each>
								</div>
								...
								<tr>
									...
									<td class="bordered">
										<xsl:choose>
											...
											<xsl:when test="dcg:ReasonCode=5">в связи с представлением декларантом после выпуска товаров документов, выданных ФОИВ, либо обращением декларанта о смене кода товаров (ТНВЭД ЕАЭС 8701-8705)</xsl:when>
