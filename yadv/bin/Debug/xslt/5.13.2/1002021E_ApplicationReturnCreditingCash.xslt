...
<xsl:stylesheet exclude-result-prefixes="cat_ru arcc" version="1.0" xmlns:arcc="urn:customs.ru:Information:CommercialFinanceDocuments:ApplicationReturnCreditingCash:5.13.2" xmlns:cat_ru="urn:customs.ru:CommonAggregateTypes:5.10.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
				...
				<div class="page">
					...
					<table width="100%">
						<tbody>
							...
							<tr align="center">
								<td class="graph8">(наименование, ИНН, КПП, адрес организации или инициала, фамилия, адрес физического лица)</td>
							</tr>
						</tbody>
					</table>
					<xsl:if test="arcc:CashType=1">
						<table width="100%">
							...
							<tbody>
								...
								<tr align="center">
									<td></td>
									<td class="graph8">(указывается основание для возврата)</td>
								</tr>
							</tbody>
						</table>
					</xsl:if>
					...
					<xsl:if test="arcc:CashType=3">
						<table width="100%">
							<tbody>
								<tr>
									<td>
										<xsl:text>в соответствии со статьей 149 Федерального закона от 27 ноября 2010 г. № 311-ФЗ "О таможенном регулировании в Российской Федерации" просит вернуть по кодю бюджетной классификации платежных документов на банковский счет </xsl:text>
										<xsl:apply-templates select="arcc:BankInformation"/>
					...
					<table width="100%">
						<tbody>
							<tr>
								...
								<td class="underlined">
									<xsl:value-of select="arcc:AmountCash"/>
							...
							<tr align="center">
								<td></td>
								<td class="graph8">(сумма денежных средство к возврату)</td>
							</tr>
						</tbody>
					</table>
					<table width="100%">
								...
								<td>
									<table>
										<tbody>
											...
											<tr align="center">
												<td></td>
												<td class="graph8">(указываются документы из предусмотренных частями 4 - 7 статьи 122 Федерального закона от 27 ноября 2010 г. № 311-ФЗ "О таможенном регулировании в Российской Федерации" и наименование и реквизиты документов, при подаче котрых в данный таможенный орган представлялись поименованные документы)</td>
											</tr>
											<tr valign="top">
												<td>2)</td>
												<td class="underlined">
					...
					<table width="100%">
						<tbody>
							...
							<tr>
								<td>
									<xsl:call-template name="Date"><xsl:with-param name="dateIn" select="arcc:ApplicantSignature/cat_ru:IssueDate"/></xsl:call-template>								
								</td>
								<td></td>
								<td></td>
							</tr>
					...
					<br/>
					<br/>
					<div class="underlined" style="width:50mm;"></div><br/>
					<span class="graph8"><sup>1</sup>Если в соответствии с законодательством Российской Федерации лицо, подавшее заявление, должно иметь печать.</span>
				</div>
			</body>
		</html>
	</xsl:template>
	...
	<xsl:template match="*" mode="document">
		...
		<xsl:if test="cat_ru:PrDocumentDate">
			...
			<xsl:call-template name="Date_2"><xsl:with-param name="dateIn" select="cat_ru:PrDocumentDate"/></xsl:call-template>
		</xsl:if>
