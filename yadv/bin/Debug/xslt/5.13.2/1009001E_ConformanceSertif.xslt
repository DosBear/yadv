...
<!-- Форма ТС: Решение Коллегии Евразийской экономической комиссии от 25 декабря 2012 г. N 293 -->
...
<xsl:stylesheet exclude-result-prefixes="cos catSert_ru cat_ru" version="1.0" xmlns:catSert_ru="urn:customs.ru:Information:SertifDocuments:SertifCommonAggregateTypesCust:5.13.1" xmlns:cat_ru="urn:customs.ru:CommonAggregateTypes:5.10.0" xmlns:cos="urn:customs.ru:Information:SertifDocuments:ConformanceSertif:5.13.2" xmlns:RUScat_ru="urn:customs.ru:RUSCommonAggregateTypes:5.13.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
					...
					<table style="border-left:solid 1pt #000000;border-top:solid 1pt #000000;border-right:solid 1pt #000000; border-bottom:solid 1pt #000000;" width="100%">
							...
							<xsl:choose>
								<xsl:when test="cos:CertificateSign=0">
									<td align="center" class="graph">
										<div class="marg-t1" style="color: silver">ТАМОЖЕННЫЙ СОЮЗ</div>
									</td>
								</xsl:when>
								...
								<table>
									<tr>
										...
										<td class="graph" style="width:120mm" valign="bottom">
											<b>
												<xsl:text>№ </xsl:text>
												<xsl:if test="cos:CertificateSign=0">ТС </xsl:if>
											</b>
											<u>
						...
						<xsl:if test="cos:SertifOrganization">
							...
							<xsl:text> </xsl:text>
								...
								<xsl:value-of select="cos:SertifOrganization/cat_ru:RKOrganizationFeatures/cat_ru:ITN/cat_ru:CategoryCode"/>/<xsl:value-of select="cos:SertifOrganization/cat_ru:RKOrganizationFeatures/cat_ru:ITN/cat_ru:KATOCode"/>/<xsl:value-of select="cos:SertifOrganization/cat_ru:RKOrganizationFeatures/cat_ru:ITN/cat_ru:RNN"/>/<xsl:value-of select="cos:SertifOrganization/cat_ru:RKOrganizationFeatures/cat_ru:ITN/cat_ru:ITNReserv"/>
									...
									<xsl:if test="cos:SertifOrganization/catSert_ru:LegalAddress or cos:SertifOrganization/catSert_ru:FactAddress"><br/></xsl:if>
						...
						<tr>
							...
							<xsl:apply-templates mode="withComma" select="cos:SertifRecipient/catSert_ru:Fax"/> 
								...
								<xsl:if test="cos:SertifRecipient/catSert_ru:LegalAddress or cos:SertifRecipient/catSert_ru:FactAddress"><br/></xsl:if>
						...
						<tr>
							...
							<xsl:apply-templates mode="withComma" select="cos:ManufacturerInf/catSert_ru:Fax"/> 
								...
								<xsl:if test="cos:ManufacturerInf/catSert_ru:LegalAddress or cos:ManufacturerInf/catSert_ru:FactAddress"><br/></xsl:if>
							...
							<xsl:if test="cos:ManufacturerInf/cat_ru:RFOrganizationFeatures">
								...
								<xsl:if test="cos:ManufacturerOrgModeCode">
									<xsl:text>; </xsl:text>
									<xsl:choose>
										<xsl:when test="cos:ManufacturerOrgModeCode='Ul'">Юридическое лицо</xsl:when>
										<xsl:when test="cos:ManufacturerOrgModeCode='Fp'">Индивидуальный предприниматель</xsl:when>
										<xsl:when test="cos:ManufacturerOrgModeCode='Al'">Иностранное юридическое лицо</xsl:when>
									</xsl:choose>
								</xsl:if>
								<xsl:if test="cos:OPF">
									<xsl:text>; организационно-правовая форма: </xsl:text>
						...
						<tr>
							<td class="graph">
								<div class="marg-top">КОД ТН ВЭД</div>
